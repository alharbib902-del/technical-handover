...
|---|----------|--------------|--------------|
| D1 | 6 Dashboards (Super Admin / Branch Manager / Finance / Affairs / Teacher / Student) | `app/(role)/dashboard/page.tsx` | كل لوحة تعرض ≥ 6 widgets فعّالة |
| D2 | مكتبة 22 تقريراً جاهزاً | `app/(admin)/reports/[reportId]/` | كلٌّ تُصدَّر Excel+PDF |
| D3 | Filters متعددة الأبعاد موحّدة | `features/reports/filters/` | 8 أبعاد على الأقل |
| D4 | تصدير Excel (مع رسوم) عبر ExcelJS | `lib/exports/excel.ts` | جدول مُنسَّق + Chart مضمَّن |
| D5 | تصدير PDF احترافي عبر Puppeteer | `lib/exports/pdf.ts` | A4 RTL + Logo + QR |
| D6 | تصدير CSV (RFC 4180) | `lib/exports/csv.ts` | UTF-8 BOM + RTL friendly |
| D7 | Materialized Views (8 views) | `supabase/migrations/0090_mv.sql` | تُحدَّث كل 15د عبر `pg_cron` |
| D8 | Realtime KPI Channels | `lib/realtime/kpi-channels.ts` | تحديث widget ≤ 2 ثانية |
| D9 | Teacher Archive Full (5 Tabs) | `app/(admin)/teachers/[id]/archive/` | Tabs 4 و5 الجديدة |
| D10 | Teacher KPIs Engine | `lib/teacher-kpis.ts` | ≥ 8 KPIs محسوبة |
...
| D13 | Anonymization Layer للتقييمات | `lib/anonymization.ts` | لا cross-reference للطالب |
| D14 | At-Risk Detection Engine | `lib/risk-engine.ts` | TypeScript + ≥ 7 أبعاد |
| D15 | جدول `student_risk_scores` + Cron يومي | `supabase/functions/risk-cron/` | يعمل 03:00 KSA يومياً |
| D16 | شاشة "الطلاب المعرّضون للخطر" | `app/(admin)/at-risk/page.tsx` | فلترة + Drill-down |
| D17 | Claude AI Explainer للـ Risk (اختياري) | `lib/risk-explainer.ts` | Toggle on/off للأدمن |
| D18 | QR-Trackable Government Reports | `app/api/student/[id]/gov-report.pdf` | QR للتحقق |
| D19 | Custom Reports Builder UI | `app/(admin)/reports/builder/` | بناء تقرير من 3+ جداول |
| D20 | Scheduled Reports (Email Delivery) | `supabase/functions/scheduled-reports/` | إرسال يومي/أسبوعي/شهري |
| D21 | RLS Policies للتقارير والتقييمات | `supabase/migrations/0095_rls_phase9.sql` | كل دور تقاريره فقط |
...
| Puppeteer (puppeteer-core + @sparticuz/chromium) | 22+ | توليد PDF احترافي من HTML |
| pg_cron | (Supabase Pro) | جدولة `REFRESH MATERIALIZED VIEW` |
| Recharts | 2.12+ | الرسوم البيانية في الـ Dashboards |
| TanStack Table v8 | 8.17+ | جداول التقارير (Pagination, Sort, Filter) |
| react-pdf | 9+ (اختياري بديل لـ Puppeteer) | معاينة PDF داخل المتصفح |
...
- **Supabase Realtime** للـ counters والإشعارات الحرجة فقط (ليس للـ aggregations).
- **ExcelJS** لـ Excel (لا تستخدم `xlsx` لأنها بطيئة للملفات الكبيرة).
- **Puppeteer + @sparticuz/chromium** لـ PDF (يعمل على Vercel Serverless).
...
- **Scheduled Reports** عبر Supabase Edge Function + `pg_cron`.
...
| البند | المواصفة |
|------|----------|
| Widget Framework | React 18 + Server Components + Suspense |
...
| Caching | `unstable_cache` (Next.js 14) + tags revalidation |
...
- `getDashboardData(role, branchId?, period?)` — Server Query موحّدة تُرجع كائناً مفصّلاً للوحة.
- `saveDashboardPreferences(userId, prefs)` — Server Action لحفظ الترتيب.
...
```typescript
// lib/realtime/use-kpi-channel.ts
export function useKpiChannel(channelName: string, onUpdate: (payload: unknown) => void) {
  useEffect(() => {
...
```sql
-- supabase/migrations/0090_materialized_views.sql
...
```typescript
// app/api/teachers/[id]/archive.pdf/route.ts
export async function GET(req: Request, { params }: { params: { id: string } }) {
  const supabase = await createServerClient();
  await assertRole(supabase, ['super_admin', 'admin', 'branch_manager']);

  const teacher = await getTeacherFullArchive(params.id);
  const html = renderTeacherArchiveHtml(teacher); // Server-side React renderToString
...
export async function submitEvaluation(input: EvaluationInput): Promise<void> {
  ...
  await supabase.from('evaluation_submissions_log').insert({
    period_id: input.periodId,
    teacher_id: input.teacherId,
    subject_id: input.subjectId,
    student_id: session.userId,
...
#### 4.10.4 Cron Job (Supabase Edge Function)

```typescript
// supabase/functions/risk-cron/index.ts
import { createClient } from 'jsr:@supabase/supabase-js@2';
import { computeRiskScore } from './risk-engine.ts';
...
Deno.serve(async (_req: Request) => {
  ...
  if (!students) return new Response('No students', { status: 200 });

  // 2. For each student, fetch all metrics
  for (const s of students) {
    const input = await fetchStudentRiskInput(supabase, s.id);
    const breakdown = computeRiskScore(input);

    // 3. Upsert
    await supabase.from('student_risk_scores').upsert({
      student_id: s.id,
    ...
    if (breakdown.band === 'red') {
      ...
      if (wasNotRed) {
        await notifyAffairs(supabase, s.id, breakdown);
      }
    }

    // 5. Append to history
    await supabase.from('student_risk_scores_history').insert({
      student_id: s.id,
      branch_id: s.branch_id,
      total_score: breakdown.totalScore,
      band: breakdown.band,
...
```
https://iims.ruwwadattaa.sa/verify/[token]
```
...
```sql
-- File: supabase/migrations/20260601000090_materialized_views.sql
-- Enable pg_cron (Supabase Pro)
...
| **Supabase Storage** | حفظ ملفات PDF/Excel المُولَّدة | عند Export |
| **Supabase Edge Functions** | Cron Risk Engine + Scheduled Reports | يومياً وكل ساعة |
| **Vercel Edge Network** | CDN للأصول الثابتة | تلقائي |
...
```typescript
// نُرسل webhook لـ:
// 1. إنشاء notification داخلية لـ Student Affairs
// 2. (مستقبلاً في المرحلة 10) إرسال WhatsApp/SMS
async function notifyAffairs(supabase: SupabaseClient, studentId: string, breakdown: RiskScoreBreakdown) {
  const { data: affairsUsers } = await supabase
    .from('users')
    .select('id, branch_id')
    .eq('role', 'student_affairs')
    .eq('branch_id', breakdown.branchId);

  for (const user of affairsUsers ?? []) {
  ...
  await supabase.from('student_risk_scores').update({ notified_at: new Date().toISOString() }).eq('student_id', studentId);
}
```
...
-- نُخفي هذا العمود في الـ Server Query عند فتح المعلم لتقييماته
...
|---------|------|------------------------|
| `computeRiskScore()` | `lib/risk-engine.test.ts` | 100% (15+ سيناريو) |
| `suggestInterventions()` | `lib/risk-engine.test.ts` | 100% |
| `generateSubmissionHash()` | `lib/anonymization.test.ts` | 100% |
| `buildCustomReportSql()` | `lib/custom-reports/builder.test.ts` | 100% |
| `exportToExcel()` | `lib/exports/excel.test.ts` | ≥ 90% |
| `exportToCsv()` | `lib/exports/csv.test.ts` | 100% (RFC 4180 fixtures) |
| Filters parsing | `features/reports/filters.test.ts` | ≥ 90% |
...
| السيناريو | الـ Spec |
|----------|---------|
| Super Admin يفتح اللوحة ويرى 6 KPIs | `e2e/dashboard-superadmin.spec.ts` |
| Branch Manager يفتح اللوحة لـ فرعه فقط | `e2e/dashboard-branch.spec.ts` |
| Finance يفتح تقرير المتأخرين ويُصدّر Excel | `e2e/report-finance.spec.ts` |
| Student يقيّم معلماً (نموذج كامل) | `e2e/evaluation-flow.spec.ts` |
| Student يحاول تقييم معلم لم يحضر مادته → فشل | `e2e/evaluation-eligibility.spec.ts` |
| الأدمن يبني تقريراً مخصصاً ويعاينه | `e2e/custom-reports-builder.spec.ts` |
| الطالب يولّد تقرير حكومي + QR يعمل | `e2e/gov-report.spec.ts` |
| فتح أرشيف معلم وتصدير PDF كامل | `e2e/teacher-archive.spec.ts` |
...
| 14:00-16:00 | DashboardWidget + KPICard + TrendChart components | React |
...
| 16:00-17:00 | Exporter للـ CSV + اختبار | `lib/exports/csv.ts` |
...
| 16:00-17:00 | Scheduled Reports Edge Function | Function |
...
| 11:00-13:00 | QR Verification page + Edge Function | Page + Function |
...
| 10:30-12:30 | Risk Engine TypeScript + Tests | `lib/risk-engine.ts` + tests |
...
| AC-9.25 | TypeScript strict — لا `any` | ESLint |
...
| R2 | **Realtime Channel يستهلك حصة Supabase Pro** | متوسطة | متوسط | حصر Realtime على KPIs الحرجة (5 channels لا أكثر) + Throttling |
| R3 | **Puppeteer على Vercel Serverless يفشل بسبب الذاكرة** | عالية | عالي | استخدام @sparticuz/chromium + تكبير Memory للـFunction (1024 MB) + Fallback لـ react-pdf |
...
| R10 | **اللوحات بطيئة على شبكات سيئة في الفروع البعيدة** | متوسطة | متوسط | Server Components + Streaming + CDN + skeleton loading جيد |
...
|--------|--------------------|-------------|
| Supabase Pro (لـ pg_cron + Realtime + Edge Functions) | $25/شهر | إلزامي لهذه المرحلة |
| Anthropic Claude API (Haiku 4.5) | ~$5-15/شهر | للـ Risk Explainer (تشغيل اختياري) |
| Vercel (تكبير Memory لـ Puppeteer) | $0 (في الـ Pro الحالي) | — |
| Resend (للـ Scheduled Reports) | $0 (حتى 3K emails) | كافٍ |
| Supabase Storage (تصدير PDF/Excel مؤقت) | $0.021/GB-month | < $1 شهرياً |
...
- [ ] `supabase/migrations/20260601000090_materialized_views.sql`
- [ ] `supabase/migrations/20260601000091_risk_detection.sql`
- [ ] `supabase/migrations/20260601000092_teacher_evaluations.sql`
- [ ] `supabase/migrations/20260601000093_dashboards_and_reports.sql`
- [ ] `supabase/migrations/20260601000094_gov_reports_correspondence.sql`
- [ ] `supabase/migrations/20260601000095_rls_phase9.sql`
- [ ] `supabase/functions/risk-cron/` (Edge Function)
- [ ] `supabase/functions/scheduled-reports/` (Edge Function)
- [ ] `src/features/dashboards/` (6 لوحات)
- [ ] `src/features/reports/` (22 تقرير + Builder)
- [ ] `src/features/evaluations/` (Student-Teacher Eval)
- [ ] `src/features/risk-detection/` (At-Risk screen)
- [ ] `src/features/teacher-archive/` (Tabs 4 و5)
- [ ] `src/lib/risk-engine.ts`
- [ ] `src/lib/anonymization.ts`
- [ ] `src/lib/exports/{excel,pdf,csv}.ts`
- [ ] `src/lib/realtime/use-kpi-channel.ts`
- [ ] `src/lib/custom-reports/builder.ts`
- [ ] `src/lib/risk-explainer.ts` (اختياري)
...
- [ ] Nafath SSO (إن اعتُمد).
- [ ] (اختياري) Self-Hosted Supabase KSA للـ PDPL الكامل.
...
4. **PDF عبر Puppeteer ثقيل** — اختبره على Vercel staging مبكراً (اليوم 6 لا اليوم 10).
