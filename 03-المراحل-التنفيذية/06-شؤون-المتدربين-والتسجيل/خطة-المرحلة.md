...
|---|----------|--------------|--------------|
| D1 | بوابة شؤون المتدربين | `src/app/(staff)/student-affairs/` | 8 شاشات تعمل + RLS صحيحة |
| D2 | بطاقة الطالب الموحّدة (7 تبويبات) | `src/components/student-card/` | كل التبويبات تعمل + تطبع PDF |
| D3 | بوابة التسجيل (Mini CRM) | `src/app/(staff)/enrollment/` | Kanban + Pipeline + Detail يعمل |
| D4 | نموذج التسجيل العام | `src/app/(public)/register/` | Zod + CAPTCHA + Rate Limiting |
...
| D11 | تقارير التسجيل (5 تقارير) | `src/app/admin/reports/enrollment/` | تقارير قابلة للتصدير CSV/PDF |
| D12 | 6 Cron Jobs | `supabase/functions/cron/` | تعمل في الإنتاج |
| D13 | 13 Migration | `supabase/migrations/` | تنجح + Rollback مختبر |
...
- ❌ **توقيع رقمي معتمد بـ نفاذ للانسحاب** — التوقيع الحالي = إقرار رقمي + IP + timestamp. PKI في مراحل لاحقة.
...
```typescript
// src/modules/withdrawal/settlement-calculator.ts
...
```typescript
// supabase/functions/cron/suspension-expiry-notifications.ts
...
```typescript
// src/modules/enrollment/state-machine/guards.ts
...
```typescript
// src/modules/enrollment/conversion/saga.ts
...
export const leadToStudentSaga = new Saga<LeadConversionContext>('lead-to-student')
  ...
  .addStep(
    ...
    async (ctx) => {
      if (ctx.authUserId) {
        await supabase.auth.admin.deleteUser(ctx.authUserId);
      }
    }
  )
  .addStep(
...
```typescript
// src/modules/withdrawal/handler.ts
...
| الحماية | الآلية |
|---------|--------|
| Rate Limiting | Upstash Redis: 5/15min/IP, 3/15min/phone |
...
| Input Sanitization | Zod + DOMPurify لأي نص |
...
|--------|-----------------|---------|
| Unit Tests | ≥ 120 اختبار | Vitest |
| Integration Tests | ≥ 40 اختبار | Vitest + Supabase Test Helpers |
| E2E Tests | ≥ 18 سيناريو | Playwright |
...
```typescript
// tests/unit/enrollment/state-machine.test.ts
...
// tests/unit/withdrawal/settlement-calculator.test.ts
...
```typescript
// tests/integration/lead-conversion-saga.test.ts
describe('Lead → Student Conversion Saga', () => {
  ...
  it('rolls back on step 4 failure (auth user creation)', async () => {
    const lead = await createTestLead({ stage: 'awaiting_payment' });
    mockSupabaseAuth.createUser.mockRejectedValue(new Error('Email exists'));
...
```typescript
// tests/e2e/phase-6/public-enrollment.spec.ts
...
// tests/e2e/phase-6/withdrawal-flow.spec.ts
...
```typescript
// tests/rls/enrollment-isolation.test.ts
...
| AT8 | محاولة Bot مع 10 تقديمات في دقيقة → Rate Limit 429 | Upstash Redis |
...
| PH6-009 | Lead State Machine (XState) + guards + actions | 10h | Lead Eng |
| PH6-010 | `POST /api/public/enrollment` + Zod schema | 6h | Full-Stack |
...
|---------|------|--------|
| فتح بطاقة طالب (P95) | < 600ms | Vercel Analytics |
...
- [ ] 0 vulnerability من مستوى High في `pnpm audit`.
...
│   ├── attachments.tsx
│   └── history.tsx
├── missing-attachments/page.tsx
├── complaints/page.tsx
├── withdrawals/page.tsx
└── suspensions/page.tsx

src/app/api/
├── public/enrollment/route.ts
├── public/enrollment/captcha-verify/route.ts
├── enrollment/leads/route.ts
├── enrollment/leads/[id]/route.ts
├── enrollment/leads/[id]/advance/route.ts
├── enrollment/leads/[id]/convert/route.ts
├── student-affairs/dashboard/route.ts
├── student-affairs/students/[id]/full-card/route.ts
├── complaints/route.ts
├── complaints/[id]/decide/route.ts
├── withdrawal/route.ts
├── withdrawal/[id]/finance-approve/route.ts
├── withdrawal/[id]/admin-approve/route.ts
├── suspension/route.ts
├── suspension/[id]/lift/route.ts
├── resume-studies/route.ts
└── cron/
    ├── missing-attachments-notify/route.ts
    ├── lead-inactivity-check/route.ts
    ├── suspension-expiry-check/route.ts
    ├── auto-cancel-stale-leads/route.ts
    ├── welcome-email-retry/route.ts
    └── onboarding-tasks-overdue/route.ts

tests/unit/enrollment/
├── state-machine.test.ts
├── conversion-saga.test.ts
└── validation.test.ts
...
tests/integration/
├── lead-conversion-saga.test.ts
├── withdrawal-flow.test.ts
├── complaint-workflow.test.ts
└── suspension-lifecycle.test.ts

tests/e2e/phase-6/
├── public-enrollment.spec.ts
├── pipeline-management.spec.ts
├── lead-to-student.spec.ts
├── student-card.spec.ts
├── withdrawal-flow.spec.ts
├── complaint-flow.spec.ts
├── suspension-lifecycle.spec.ts
├── missing-attachments.spec.ts
└── reports.spec.ts

tests/rls/
├── enrollment-isolation.test.ts
├── complaints-anonymity.test.ts
└── withdrawal-isolation.test.ts
...
# Email (Resend)
RESEND_API_KEY=...
RESEND_FROM_EMAIL=no-reply@ruwwadattaa.sa
