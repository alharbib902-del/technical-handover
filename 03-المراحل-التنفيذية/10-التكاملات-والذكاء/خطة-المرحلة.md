...
> **المدة المُقدّرة:** ستة أسابيع (30 يوم عمل) — قابلة للتمديد إلى 35 يوماً عند تأخّر اعتماد قوالب WhatsApp أو OTP/SMS
...
المرحلة العاشرة — **التكاملات الخارجية + الذكاء + المستقبل (External Integrations + AI + Future)** — هي **المرحلة الختامية** من مشروع النظام IIMS. بعد أن أكملت المراحل التسع السابقة كل البنية الأساسية (التأسيس، الاختبارات، الطلاب، المالية والحجب، الطلبات والخطابات، شؤون المتدربين، الأكاديمي اليومي، تكامل TVTC، التقارير والأرشيف)، يأتي دور هذه المرحلة لتُغلِف المنتج بطبقة من التكاملات الخارجية الذكية التي تربط النظام بالعالم الخارجي (WhatsApp، SMS، Email، OTP/SMS)، وتُضيف ذكاءً اصطناعياً يُخفّف العبء عن شؤون المتدربين (AI Chatbot)، وتُمدّ النظام للمستقبل عبر بوّابة الخريجين والدورات التطويرية المستمرة. هذه المرحلة لا تُضيف موديولات داخلية جوهرية جديدة، لكنّها **تُحوّل النظام من "نظام إدارة معاهد" إلى منصة شاملة متصلة بكل مزوّدي الخدمة في المملكة**.

السمة الفارقة لهذه المرحلة هي **التحوّل من الانعزال إلى الاتصال**: كل البيانات التي بُنيت عبر تسع مراحل (~24 موديول، 50+ جدول، 300+ Livewire Action / Laravel Controller) كانت محبوسة داخل النظام تنتظر من المستخدم أن يدخل ليراها. الآن سيخرج النظام بنفسه إلى الطالب — عبر WhatsApp ليُذكّره بقسطه، عبر Email ليُرسل خطابه الرسمي PDF، عبر AI Chatbot ليجيب عن سؤاله في الثانية الواحدة، عبر OTP/SMS ليُسهّل دخوله دون تذكّر كلمة المرور، وعبر بوّابة الخريجين ليُحافظ على علاقة دائمة تتجاوز التخرّج. هذا التحوّل ليس "تجميلياً" — بل يُتوقّع منه **تقليل عبء شؤون المتدربين بـ60-70%** عبر AI Chatbot وحده، و**تقليل ديون التحصيل بـ20-25%** عبر تذكيرات WhatsApp الذكية، و**فتح مصدر دخل مستدام** للمعهد عبر الدورات التطويرية.

نُجري في هذه المرحلة سبعة تكاملات خارجية رئيسية: **(1) WhatsApp Business API** عبر Unifonic مع ثمانية قوالب معتمدة (payment_reminder, request_confirmation, ...) وإدارة Session Window 24h وFallback ذكي، **(2) SMS Gateway** عبر Unifonic للاستخدام الحرج فقط (OTP، تنبيهات حرمان) مع تتبّع التكلفة، **(3) Email Service** عبر Resend مع قوالب Livewire/Blade Email عربية RTL مع تتبّع الفتح والنقر، **(4) AI Chatbot للطلاب** مدعوماً بـClaude API + Function Calling يُجيب عن أسئلة الرصيد والاختبار والطلب والحضور 24/7، **(5) OTP/SMS Authentication** عبر OIDC مع ربط الهويات Multi-Provider، **(6) بوّابة الخريجين** كموقع مستقل بشبكة مهنية واستبيانات متابعة، **(7) الدورات التطويرية المستمرة** للخريجين كمصدر دخل مستدام مهم للطلاب الموظفين الحكوميين الباحثين عن الترقيات.

تشمل المرحلة أيضاً **بنية إشعارات موحّدة Smart Routing** تختار القناة الأمثل لكل طالب احتراماً لتفضيلاته (User Preferences)، و**Quiet Hours** (لا إرسال بين 11 PM و7 AM)، و**Anti-Spam** (حد أقصى لعدد الإشعارات/يوم)، و**طبقة Webhooks واردة موحّدة** بـSignature Verification وIdempotency Keys، و**أمان تكاملات صارم** عبر API Keys في Vault مع Rotation كل 3 أشهر وCircuit Breaker لكل مزوّد. كل تكامل يمر عبر **Adapter Pattern** يضمن أن استبدال أي مزوّد لاحقاً (Unifonic → مزوّد بديل مثلاً) لا يلمس المنطق التجاري.

ميزة AI Chatbot تستحق إفراد فقرة لها: نستخدم **Claude API** مع **Function Calling** عبر أربع دوال أساسية (`getStudentBalance`, `getNextExam`, `getRequestStatus`, `getAttendanceRate`) تُنفَّذ على Livewire Actions / Laravel Controllers الجاهزة من المراحل 4 و5 و7. السياق محصور بـ`student_id` الجلسة (لا يستطيع طالب رؤية بيانات طالب آخر)، والـPrompt System الأساسي يُلزم البوت بثلاث قواعد ذهبية: **(أ) لا يصدر قرارات** (لا فك حجب، لا تعديل بيانات) — فقط معلومات، **(ب) يحوّل لموظف بشري** عند 3 محاولات فاشلة أو عند طلب صريح أو عند سؤال خارج النطاق، **(ج) يحفظ كل محادثة** في `chatbot_conversations` للتحسين والـAudit. التوقّع الواقعي: 60-70% من تذاكر شؤون المتدربين الحالية (التي 80% منها أسئلة متكررة بسيطة) تُحلّ بـ Chatbot دون تدخّل بشري.

في نهاية الستة أسابيع، سنُسلّم: **سبع تكاملات إنتاجية مُختبَرة**، **8 قوالب WhatsApp معتمدة من Meta**، **AI Chatbot يُجيب 80%+ من الأسئلة الشائعة بدقّة 95%+**، **OTP/SMS Authentication يعمل** (إن وافق العميل ووصلت موافقة NIC) أو Mock جاهز للإنتاج، **بوّابة خريجين مستقلة** بشبكة مهنية واستبيانات، **كاتالوج دورات تطويرية** قابل للتسجيل، **بنية إشعارات موحّدة** عبر القنوات الأربع، **Vault للأسرار** بـRotation، **خارطة طريق للمستقبل** للمراحل 11+ (Multi-Tenancy، Mobile، Auto-Grading)، و**وثيقة Go-Live** كاملة. بهذا التسليم يكون مشروع النظام IIMS **مُكتمَلاً ومُجهّزاً للإطلاق الإنتاجي الكامل** على 1000 طالب موزّعين على 4 فروع، مع جاهزية معمارية للتوسع لـ10,000 طالب و8 فروع، وقابلية للتسويق كمنتج SaaS للمعاهد الأخرى مستقبلاً.
...
| O5 | اعتماد OTP/SMS أو جاهزية الـAdapter | OIDC يعمل مع Mock + جاهز للإنتاج عند موافقة NIC |
...
|---|----------|--------------|--------------|
| D1 | WhatsApp Adapter كامل | `app/Services/integrations/whatsapp/` | يُرسل + يستقبل + يتتبّع 5 statuses |
| D2 | 8 قوالب WhatsApp معتمدة | Meta Business Manager | كل القوالب status=APPROVED |
| D3 | جدول `whatsapp_messages` | `database/migrations/0103_whatsapp.sql` | RLS + indexes + triggers |
| D4 | SMS Adapter | `app/Services/integrations/sms/` | OTP يصل خلال 30 ثانية |
| D5 | جدول `sms_messages` + cost tracking | `database/migrations/0104_sms.sql` | تجميع التكلفة اليومي/الشهري |
| D6 | Resend Email Adapter | `app/Services/integrations/email/` | يُرسل + يتتبّع opens/clicks |
| D7 | 12 قالب Blade Mailable | `resources/views/emails/*.blade.php` + `app/Mail/*` | كل القوالب عربية RTL + Cairo |
| D8 | جدول `email_logs` | `database/migrations/0105_email.sql` | يستوعب 8 statuses |
| D9 | AI Chatbot Frontend | `app/Livewire/chatbot/` | Chat UI في بوّابة الطالب |
| D10 | AI Chatbot Backend (Claude + Tools) | `app/Services/ai/` | 4 functions + Anti-abuse |
| D11 | جدول `chatbot_conversations` | `database/migrations/0106_chatbot.sql` | يحفظ كل رسالة + التحويلات |
| D12 | OTP/SMS OIDC Adapter | `app/Services/integrations/otp_sms/` | يعمل مع Mock + جاهز للحقيقي |
| D13 | جدول `auth_identities` | `database/migrations/0107_identities.sql` | يدعم Multi-Provider |
| D14 | بوّابة Alumni (subdomain) | `apps/alumni/` أو `/alumni/*` | تسجيل + بحث + استبيانات |
| D15 | جداول Alumni | `database/migrations/0108_alumni.sql` | 6 جداول مع RLS |
| D16 | كاتالوج Continuing Education | `app/Livewire/continuing-ed/` | عرض + تسجيل + دفع |
| D17 | جداول Continuing Education | `database/migrations/0109_continuing_ed.sql` | يستفيد من بنية الاختبارات |
| D18 | بنية Notifications موحّدة | `app/Services/notifications/` | Smart Routing + Quiet Hours |
| D19 | شاشة User Preferences | `/student/settings` | اللغة + القنوات + Theme |
| D20 | طبقة Webhooks واردة موحّدة | `app/Http/Controllers/Api/webhooks/[provider]/` | Signature + Idempotency + Queue |
| D21 | Vault للأسرار + Rotation | Laravel + PostgreSQL Vault أو Doppler | كل API Key بـrotation policy |
| D22 | Circuit Breakers لكل مزوّد | `src/lib/cIrcuitBreaker.php` | Open/Half/Closed states |
| D23 | جدول `integration_errors` | `database/migrations/0110_integration_errors.sql` | يتتبّع كل خطأ |
...
| **Domain للإيميل** | المعهد | `noreply@iims.sa` + SPF + DKIM + DMARC | اليوم 3 |
| **Anthropic Claude API** | المطوّر | API Key + Pay-as-you-go enabled + budget alert | اليوم 1 |
| **NIC OTP/SMS Registration** | المعهد + قانوني | Application مُقدَّم + Test Environment access | اليوم 1 (الموافقة 30-60 يوم) |
| **Doppler / Laravel + PostgreSQL Vault** | المطوّر | Account + Project | اليوم 2 |
| **Subdomain للخريجين** (`alumni.iims.sa`) | المعهد | DNS مُعدّ | اليوم 5 |
...
| `resend` | ≥ 4.0 | Email |
| `@react-email/components` | ≥ 0.0.30 | Livewire/Blade Email components |
| `react-email` | ≥ 3.0 | محرّك القوالب |
| `openid-client` | ≥ 5.6 | OIDC لOTP/SMS |
...
- **OTP/SMS OIDC فقط** (وليس SAML — أبسط وأحدث).
- **بوّابة Alumni على Subdomain** (`alumni.iims.sa`) — أنظف من path-based.
...
- **API Keys في Vault** (Laravel + PostgreSQL Vault للبداية، Doppler إن احتجنا cross-env).
...
#### 4.1.5 الـAPIs / Livewire Actions / Laravel Controllers
...
#### 4.2.5 الـAPIs / Livewire Actions / Laravel Controllers
...
#### 4.3.1 الوصف
بناء طبقة Adapter لـEmail عبر Resend مع 12 قالب Livewire/Blade Email عربي RTL، تشمل تأكيد التسجيل، إعادة كلمة المرور، الخطابات الرسمية كـPDF، التقارير الدورية، والإشعارات الأقل أولوية.
...
| Domain | `noreply@iims.sa` |
| Authentication | SPF + DKIM + DMARC |
| Templates Engine | Livewire/Blade Email + Cairo font |
...
#### 4.3.6 الـAPIs / Livewire Actions / Laravel Controllers
- `sendEmail(to, templateName, vars, attachments?)` — Livewire Action / Laravel Controller عامة.
...
### 4.5 موديول OTP/SMS Authentication

#### 4.5.1 الوصف
تكامل OIDC مع بوّابة OTP/SMS للهوية الرقمية، يسمح للطلاب بتسجيل الدخول عبر تطبيق OTP/SMS، مع ربط متزامن (Linked Identities) للحساب الأصلي، وMock للتطوير قبل اعتماد NIC.

#### 4.5.2 User Stories
- **US-10.5.1** كطالب، أريد تسجيل الدخول بOTP/SMS بدون تذكّر كلمة مرور، لراحة أكبر.
- **US-10.5.2** كنظام، أريد التحقق من هوية الطالب عبر OTP/SMS قبل الانسحاب، لمنع الاحتيال.
- **US-10.5.3** كطالب، أريد ربط OTP/SMS بحسابي الموجود (لا حساب جديد)، للاستمرارية.
- **US-10.5.4** كنظام، أريد مزامنة الاسم والجنسية وتاريخ الميلاد من OTP/SMS تلقائياً، لتقليل الأخطاء.
- **US-10.5.5** كمطوّر، أريد Mock OTP/SMS في التطوير قبل موافقة NIC، لإكمال الكود.
...
| Authorization Endpoint | `https://otp_sms.gov.sa/oauth/authorize` (إنتاج) |
| Token Endpoint | `https://otp_sms.gov.sa/oauth/token` |
| Scopes | `openid profile national_id` |
| Linked Identities | جدول `auth_identities` يدعم providers متعددة |
| Mock | `OTP/SMSMockAdapter` للتطوير |

#### 4.5.4 التغييرات في الـDatabase
- جدول `auth_identities` (راجع القسم 5).
- إضافة حقول `national_id`, `otp_sms_verified_at` لـ`users`.

#### 4.5.5 الـAPIs / Livewire Actions / Laravel Controllers
- `initiateOTP/SMSLogin()` → redirect إلى OTP/SMS.
- `handleOTP/SMSCallback(code, state)` → exchange + link.
- `verifyIdentityViaOTP/SMS(studentId)` → للعمليات الحساسة.
...
#### 4.6.1 الوصف
موقع مستقل (subdomain `alumni.iims.sa`) للخريجين بعد الإكمال، يشمل: ملف خريج دائم، شبكة مهنية، استبيانات متابعة المسار، إعلانات الترقيات/فرص التطوير من جهات حكومية شريكة، وشهادة "خريج" قابلة للتحقق.
...
| البند | المواصفة |
|------|----------|
| النشر | Subdomain منفصل (`alumni.iims.sa`) |
| المعمارية | Laravel Routes + Filament Panels (نفس المنصة) |
...
#### 4.6.5 الـAPIs / Livewire Actions / Laravel Controllers
...
#### 4.7.6 الـAPIs / Livewire Actions / Laravel Controllers
...
- **Queue** للمعالجة (pg_cron / Laravel Horizon).
...
#### 4.11.1 المكوّنات
- **API Keys في Vault** (Laravel + PostgreSQL Vault للبداية).
- **Rotation** كل 3 أشهر إلزامي.
...
CREATE TABLE email_logs (
  ...
  from_email VARCHAR(255) NOT NULL DEFAULT 'noreply@iims.sa',
...
CREATE TABLE auth_identities (
  ...
  provider VARCHAR(50) NOT NULL
    CHECK (provider IN ('email','otp_sms','google','apple','microsoft')),
...
ALTER TABLE users ADD COLUMN otp_sms_verified_at TIMESTAMPTZ;
ALTER TABLE users ADD COLUMN otp_sms_verified_data JSONB;
```
...
| Alumni Home | `alumni.iims.sa/` | Alumni | الصفحة الرئيسية |
...
|---------|--------|------|
| `<ChatbotWidget>` | `app/Livewire/chatbot/chatbot-widget.blade.php (+ Livewire class)` | Floating chat |
| `<ChatBubble>` | `app/Livewire/chatbot/chat-bubble.blade.php (+ Livewire class)` | رسالة في المحادثة |
| `<ToolCallCard>` | `app/Livewire/chatbot/tool-call-card.blade.php (+ Livewire class)` | عرض نتيجة دالة |
| `<TransferBanner>` | `app/Livewire/chatbot/transfer-banner.blade.php (+ Livewire class)` | إحالة بشري |
| `<DeliveryStatusBadge>` | `src/components/comms/delivery-status-badge.blade.php (+ Livewire class)` | حالة WhatsApp |
| `<NotificationChannelToggle>` | `app/Livewire/preferences/notification-channel-toggle.blade.php (+ Livewire class)` | تبديل قناة |
| `<QuietHoursPicker>` | `app/Livewire/preferences/quiet-hours-picker.blade.php (+ Livewire class)` | اختيار الساعات |
| `<AlumniCard>` | `app/Livewire/alumni/alumni-card.blade.php (+ Livewire class)` | بطاقة خريج |
| `<OpportunityCard>` | `app/Livewire/alumni/opportunity-card.blade.php (+ Livewire class)` | بطاقة فرصة |
| `<CECourseCard>` | `app/Livewire/continuing-ed/c-e-course-card.blade.php (+ Livewire class)` | بطاقة دورة |
| `<IntegrationHealthCard>` | `app/Livewire/integrations/health-card.blade.php (+ Livewire class)` | حالة مزوّد |
| `<OTP/SMSButton>` | `app/Livewire/auth/OTP/s-m-s-button.blade.php (+ Livewire class)` | زر OTP/SMS الرسمي |
...
| **OTP/SMS (NIC)** | OIDC | SSO | متوسط | Production (عند موافقة NIC) |
| **Meta Business** | WhatsApp Templates | اعتماد القوالب | حرج | Production |
| **Laravel + PostgreSQL Vault** | Secrets | إدارة المفاتيح | حرج | Production |

### 7.2 WhatsApp Integration — Adapter Pattern

```typescript
// app/Services/integrations/whatsapp/tYpes.php
...
```typescript
// app/Services/integrations/whatsapp/uNifonicAdapter.php
...
```typescript
// app/Services/integrations/sms/uNifonicSmsAdapter.php
...
```typescript
// app/Services/integrations/email/rEsendAdapter.php
...
export class ResendEmailAdapter implements IEmailService {
  ...
  constructor(private config: { apiKey: string; from: string }, private repo: EmailRepository) {
    this.resend = Resend::client(config.apiKey);
  }
...
### 7.6 OTP/SMS OIDC Integration

```typescript
// app/Services/integrations/otp_sms/oIdcAdapter.php
import { Issuer, generators } from 'openid-client';

export class OTP/SMSOIDCAdapter implements IOTP/SMSService {
  private client: any;

  async initialize(): Promise<void> {
    const issuer = await Issuer.discover('https://otp_sms.gov.sa');
    this.client = new issuer.Client({
      client_id: process.env.OTP_SMS_CLIENT_ID!,
      client_secret: process.env.OTP_SMS_CLIENT_SECRET!,
      redirect_uris: [`${process.env.APP_URL}/api/auth/otp_sms/callback`],
      response_types: ['code'],
    });
  }
  ...
  async handleCallback(input: {
    ...
    codeVerifier: string;
  }): Promise<OTP/SMSProfile> {
    if (input.state !== input.expectedState) {
      throw new Error('Invalid state — possible CSRF');
    }

    const tokenSet = await this.client.callback(
      `${process.env.APP_URL}/api/auth/otp_sms/callback`,
...
// app/Services/integrations/otp_sms/mOck.php
export class OTP/SMSMockAdapter implements IOTP/SMSService {
  initiateLogin() {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('Mock only allowed in dev/staging');
    }
    return {
      url: '/dev/otp_sms-mock?return=/api/auth/otp_sms/callback',
      state: 'mock-state',
      nonce: 'mock-nonce',
  ...
  async handleCallback(): Promise<OTP/SMSProfile> {
    return {
...
sequenceDiagram
    ...
    participant N as بوابة OTP/SMS
    participant A as OTP/SMS App
    participant DB as Database

    U->>App: زر "تسجيل الدخول بOTP/SMS"
    App->>App: generate state + nonce + codeVerifier
    App->>App: save in encrypted cookie
    App-->>U: redirect to OTP/SMS
    U->>N: GET /authorize?state=..&code_challenge=..
    N-->>U: شاشة "افتح تطبيق OTP/SMS"
    ...
    alt Found
        App->>DB: link auth_identity (otp_sms)
    else Not Found
...
```typescript
// app/Services/notifications/dIspatcher.php
...
```typescript
// src/lib/sEcrets.php
// Laravel DB - استعمل Illuminate Support Facades DB / Eloquent Models

class SecretManager {
  private cache = new Map<string, { value: string; expiry: Date }>();
  private DB $db; // Laravel DB Connection (Service Role context)

  async get(key: string): Promise<string> {
    const cached = this.cache.get(key);
    if (cached && cached.expiry > new Date()) return cached.value;

    const { data, error } = await app("db")->statement('vault.read_secret', {
      secret_name: key,
    });
  ...
  async rotate(key: string, newValue: string): Promise<void> {
    await app("db")->statement('vault.update_secret', {
      secret_name: key,
      new_value: newValue,
    });
...
```typescript
// app/Http/Controllers/Api/webhooks/[provider]/rOute.php
...
| OTP/SMS state mismatch (CSRF) | منخفضة | حرج | state + nonce + PKCE |
| Alumni account takeover | متوسطة | عالي | OTP/SMS + 2FA للعمليات الحساسة |
...
### 9.4 اختبارات OTP/SMS

```typescript
describe('OTP/SMSOIDCAdapter', () => {
...
### Sprint 5 (أسبوع 5) — OTP/SMS + Alumni Portal

**الأهداف:**
- OTP/SMS OIDC Adapter + Mock.
- زر "تسجيل الدخول بOTP/SMS" في `/login`.
- ربط Identities.
...
**المخرجات:**
- OTP/SMS Mock يعمل end-to-end (انتظار NIC للحقيقي).
...
### 11.3 Email
- [x] 12 قالب Livewire/Blade Email جاهز بـRTL + Cairo.
...
| R2 | NIC ترفض الـapplication لـOTP/SMS | متوسطة | متوسط | الـMock يكفي للإطلاق + إعادة التقديم |
...
| Email (Resend) + 12 Livewire/Blade Email templates | 4 |
...
### الكود
- [ ] `app/Services/integrations/whatsapp/` كامل (Adapter + Tests).
- [ ] `app/Services/integrations/sms/` كامل.
- [ ] `app/Services/integrations/email/` كامل.
- [ ] `app/Services/integrations/otp_sms/` كامل (Adapter + Mock).
- [ ] `app/Services/ai/` كامل (Service + Tools + Prompts).
- [ ] `app/Services/notifications/` (Dispatcher + Scheduler).
- [ ] `app/Livewire/chatbot/` (UI).
- [ ] `app/Livewire/alumni/` (UI).
- [ ] `app/Livewire/continuing-ed/` (UI).
- [ ] `app/Livewire/preferences/` (UI).
- [ ] `apps/alumni/` (Subdomain).
- [ ] `app/Http/Controllers/Api/webhooks/[provider]/rOute.php`.
- [ ] `src/lib/cIrcuitBreaker.php`.
- [ ] `src/lib/sEcrets.php`.
- [ ] `resources/views/emails/*.blade.php` + `app/Mail/*` (12 قالب Blade Mailable).

### قاعدة البيانات
- [ ] `database/migrations/0103_whatsapp.sql`.
- [ ] `database/migrations/0104_sms.sql`.
- [ ] `database/migrations/0105_email.sql`.
- [ ] `database/migrations/0106_chatbot.sql`.
- [ ] `database/migrations/0107_identities.sql`.
- [ ] `database/migrations/0108_alumni.sql`.
- [ ] `database/migrations/0109_continuing_ed.sql`.
- [ ] `database/migrations/0110_integration_errors.sql`.
- [ ] `database/migrations/0111_notifications.sql`.
...
- [ ] OTP/SMS application مُقدَّم لـNIC.
...
- [ ] `docs/integrations/otp_sms-runbook.md`.
...
- [ ] Laravel Pulse / Telescope للأداء.
...
#### البنية التحتية
- [ ] PostgreSQL ذاتي الاستضافة على VPS سعودي مُجَهَّز (PDPL).
- [ ] DNS + SSL + CDN.
...
- استخدام نفس Livewire Actions / Laravel Controllers.
...
├── 24 موديول وظيفي
├── 50+ جدول
├── 300+ Livewire Action / Laravel Controller
├── 200+ شاشة
├── 7 تكاملات خارجية
├── 1 AI Chatbot
├── 4 فروع × 1000 طالب
...
4. **OTP/SMS مرن** — إن لم تصل موافقة NIC، الـMock يكفي للإطلاق. لا توقّف المرحلة بسبب NIC.
