...
- ❌ **التكامل مع OTP/SMS Authentication** — مؤجَّل للمرحلة 9 (مكمّل لكن منفصل).
...
1. **Interface هو نفسه، التطبيق هو ما يتغيّر:** نحن لا نلمس أي ملف خارج `app/Services/integrations/tvtc/adapters/rEal.php` (أو ما يقاربه). كل الموديولات الأخرى (Certificates, Students, Grades) تستهلك `ITVTCAdapter` ولا تعرف إن كان Mock أو Real. هذا يضمن أن **تبديل البيئات** بين التطوير والإنتاج لا يكسر شيئاً.
...
| Background Jobs Infrastructure (pg_cron أو Laravel Horizon) | المرحلة 4 | ✅ — مُستخدَم للمالية |
| Laravel + PostgreSQL Vault لحفظ الـsecrets | المرحلة 1 | ✅ — للـ ACCOUNTING_API_KEY |
...
required_services:
  ...
  - name: Laravel + PostgreSQL
    ...
    features: [Postgres, Laravel Jobs / Scheduled Tasks, Vault, pg_cron, pg_net]
...
- **Queue-First دائماً:** لا syncronous calls إلى TVTC من Livewire Actions / Laravel Controllers. كل شيء يمرّ بـ Queue.
...
```
app/Services/integrations/tvtc/
├── Types.php                    # كل الـtypes (موجود من المرحلة 2، نوسّعه)
├── Interface.php                # ITVTCAdapter (موجود، نوسّعه)
├── adapters/
│   ├── Mock.php                 # MockTVTCAdapter (يبقى للاختبار + Local Dev)
│   ├── MockV2.php              # محاكي محسَّن للـ Sandbox-like behavior
│   └── Real.php                 # RealTVTCAdapter ⭐ الجديد
├── HttpClient.php              # axios + mTLS + Retry + CB
├── Resilience.php               # CircuitBreaker + Retry + DLQ (مشترك مع المحاسبي)
├── auth/
│   ├── Oauth2Strategy.php      # إن استُخدم OAuth2
│   ├── ApiKeyStrategy.php     # إن استُخدم API Key
│   └── MtlsStrategy.php        # إن استُخدم Mutual TLS
├── mappers/
│   ├── trainee.Mapper.php       # نظام → TVTC payload
│   ├── grade.Mapper.php
│   ├── attendance.Mapper.php
│   ├── certificate.Mapper.php
│   └── program.Mapper.php
├── validators/
│   ├── trainee.Schema.php       # Form Requests + Validation Rules للـ outbound
│   ├── grade.Schema.php
│   └── ...
├── workers/
│   ├── SyncWorker.php          # Pull data من TVTC
│   ├── PushWorker.php          # Push المعلَّق
│   ├── Reconciliation.php       # Daily 04:00
│   ├── Backfill.php             # One-shot للبيانات القديمة
│   └── HealthCheck.php         # كل دقيقتين
├── Queue.php                    # Queue manager (يستخدم tvtc_outbound_queue)
...
```typescript
// app/Services/integrations/tvtc/iNterface.php
...
```typescript
// app/Services/integrations/tvtc/adapters/rEal.php
...
```typescript
// app/Services/integrations/tvtc/ServiceProvider.php (DI binding)
...
```typescript
// app/Services/integrations/tvtc/hTtpClient.php
...
- الممارسات المعيارية في APIs الحكومية السعودية (OTP/SMS، يقين، أبشر، GOSI).
...
```typescript
// app/Services/integrations/tvtc/mappers/trainee.Mapper.php
...
```sql
-- database/migrations/0080_tvtc_students_extensions.sql
...
```typescript
// app/Services/integrations/tvtc/workers/gRadeDispatchDecision.php
...
```json
{
  ...
  "verify_url": "https://iims.sa/verify/RUW-2026-001234"
}
```
...
```typescript
// app/admin/integrations/tvtc/dashboard/page.blade.php (Livewire view)
...
```typescript
// app/Services/integrations/tvtc/aLerts.php
...
$schedule->job(new TvtcReconciliationJob)
    ->dailyAt('04:00')
    ->name('tvtc-daily-reconciliation')
    ->withoutOverlapping();
```

```typescript
// app/Jobs/Scheduled/tvtc-reconciliation/iNdex.php
// Laravel DB - استعمل Illuminate Support Facades DB / Eloquent Models
import { getTVTCAdapter } from '@/lib/integrations/tvtc/di';

Deno.serve(async () => {
  // Laravel: استعمل DB::table() أو Eloquent Models مباشرة
  const tvtc = getTVTCAdapter();
  const reconciliationId = crypto.randomUUID();

  await DB::table('tvtc_reconciliation_log').insert({
    id: reconciliationId,
  ...
  const { data: missingGrades } = await DB
    .from('grades')
  ...
  await DB::table('tvtc_reconciliation_log').update({
    status: 'completed',
...
```typescript
// app/Services/integrations/tvtc/rEtryPolicy.php
...
| 2 | Mutual TLS (إن طلبه TVTC) | شهادة عميل + مفتاح خاص في Laravel + PostgreSQL Vault |
| 3 | Certificate Pinning | فحص fingerprint السيرفر في `checkServerIdentity` |
| 4 | API Keys في Vault لا في env files | استخدام `DB::statement('vault.get_secret')` |
...
| 10 | Rate Limiting على واجهة التحقق العامة | Cloudflare + Laravel job / scheduled task (100 req/min/IP) |
...
-- Retention: 90 يوم (دالة SQL مستدعاة من Laravel Scheduler)
...
```php
// app/Console/Kernel.php — Laravel Scheduler
$schedule->job(new TvtcHealthCheckJob)->everyTwoMinutes()->name('tvtc-health-check');
$schedule->call(fn () => DB::statement('SELECT cleanup_tvtc_health();'))
    ->dailyAt('05:00')
    ->name('tvtc-health-cleanup');
```
...
```php
// app/Console/Kernel.php — Laravel Scheduler (يومياً 23:55)
$schedule->job(new TvtcComplianceSnapshotJob)
    ->dailyAt('23:55')
    ->name('tvtc-compliance-snapshot')
    ->withoutOverlapping();
```
...
- استخدام نفس مكتبة المكوّنات (Filament 3 UI).
- ألوان الحالات:
  - أخضر: `registered` / `active` / `healthy`.
  - أصفر: `pending` / `degraded`.
  - أحمر: `rejected` / `down` / `dead_letter`.
- إشعارات realtime على شاشة الأدمن عبر Laravel Reverb (WebSockets).
...
```typescript
// app/Services/integrations/tvtc/auth/oAuth2Strategy.php
import { AxiosInstance } from 'axios';
...
```typescript
// app/Services/integrations/tvtc/auth/aPiKeyStrategy.php
...
```typescript
// app/api/webhooks/tvtc/rOute.php
import { NextRequest, NextResponse } from 'laravel/server';
...
- تخزين في **Laravel + PostgreSQL Vault** (مشفّر at-rest).
- تدوير سنوي.
...
```typescript
// tests/integrations/tvtc/trainee.mapper.Test.php
...
```typescript
// tests/integrations/tvtc/retry-policy.Test.php
...
```typescript
// tests/integrations/tvtc/contracts/trainee-registration.Test.php
import Ajv from 'ajv';
...
```typescript
// tests/integrations/tvtc/real-adapter.integration.Test.php
...
| 2-3 | بناء `HttpClient.php` مع mTLS + Pinning | Module + tests |
| 3-4 | بناء `auth/oAuth2Strategy.php` و `auth/aPiKeyStrategy.php` | Modules + tests |
...
|------|-------------------|
| VPS KSA (Hostinger / Contabo) (مشترك) | $25 |
| Sentry Pro (مشترك) | $26 |
| Background Workers (VPS KSA (Hostinger / Contabo) Pro) | $20 (موجود) |
...
- [ ] `app/Services/integrations/tvtc/` (كامل)
- [ ] `app/Filament/Admin/Resources/TvtcResource.php / Pages/` (14 شاشة)
- [ ] `app/Http/Controllers/Api/webhooks/tvtc/rOute.php`
- [ ] `database/migrations/0080_*.sql` → `0089_*.sql` (10 migrations)
- [ ] `app/Jobs/Scheduled/Tvtc*.php` (5 Laravel jobs / scheduled tasks)
