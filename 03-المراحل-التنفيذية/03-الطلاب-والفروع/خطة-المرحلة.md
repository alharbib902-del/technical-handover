...
المرحلة 3 هي **القلب التشغيلي** لالنظام. بعد أن انتهت المرحلة 1 من تأسيس البنية التحتية (Laravel 11 + Laravel + PostgreSQL + Auth + Schema الأساسي + Seed للفروع الأربعة) وانتهت المرحلة 2 من بناء بنية الاختبارات (Pest + Laravel Dusk + RBAC tests skeleton)، تأتي هذه المرحلة لبناء **الكتلة الوظيفية الأكبر في النظام**: ملف الطالب الكامل + إدارة الفروع التشغيلية + State Machine الحرج الذي يربط الأكاديمي بالمالي.
...
2. **State Machine في Spatie Laravel Model States** مع 8 حالات + 14 transition + 12 guard + 8 action.
3. **شاشات Laravel 11** (Laravel Routes + Filament Panels) لقائمة الطلاب + ملف الطالب + إضافة/تعديل + نقل بين الفروع + تغيير الحالة + Bulk Import + Audit Timeline.
4. **APIs/Livewire Actions / Laravel Controllers** لكل العمليات (CRUD + Bulk + Transfer + Status Change + Export).
...
- بناء State Machine بـ **Spatie Laravel Model States v5** مع `setup()` + Actor model.
...
- تخزين في Spatie Media Library Bucket باسم `students` مع مفتاح `{student_id}/{attachment_type}/{filename}`.
...
| تكامل OTP/SMS للتحقق من الهوية | OTP/SMS مؤجّلة (قرار معماري) | المرحلة 10 (اختياري) |
...
|---------|--------|----------------|
| مشروع Laravel + PostgreSQL مُعدّ + Auth جاهز | المرحلة 1 | ✅ |
| جداول `users`, `branches`, `audit_log` موجودة | المرحلة 1 | ✅ |
| Laravel 11 Laravel Routes + Filament Panels + PHP 8.3 strict | المرحلة 1 | ✅ |
| RLS مُفعّل على كل الجداول | المرحلة 1 | ✅ |
| Tailwind + Filament 3 UI + خطوط Cairo/IBM Plex Sans Arabic | المرحلة 1 | ✅ |
| Pest + Laravel Dusk + RBAC test harness | المرحلة 2 | ✅ |
| Storybook مُهيّأ | المرحلة 2 | ✅ |
| CI/CD pipeline (GitHub Actions) | المرحلة 1 | ✅ |
| Laravel Pint + PHPStan/Larastan + Lefthook / Git Hooks pre-commit | المرحلة 1 | ✅ |
| Spatie Media Library Bucket `students` + سياسات الرفع | المرحلة 1 | ⚠️ نُنشئ هنا لو غير موجود |

### 3.2 التبعيات الناعمة (Soft Dependencies)

- مكتبة Spatie Laravel Model States v5 (نُضيفها في هذه المرحلة).
...
- مكتبة `Filament Tables` لجداول الطلاب الكبيرة مع pagination/sorting/filtering.
...
| Race condition في تغيير الحالة | استخدام Spatie Laravel Model States مع `actor.send` المتسلسل + DB lock |
...
CREATE TABLE student_attachments (
    ...
    file_path TEXT NOT NULL,  -- المسار في Spatie Media Library
...
**أداة التنفيذ:** **Spatie Laravel Model States v5** (`setup()` API + Actor model).
...
> **التنفيذ:** كل guard دالة Pure في `src/lib/student-state-machine/gUards.php` بتوقيع `({ context, event }) => boolean`. يتم اختبار كل guard بـ ≥ 2 cases (positive + negative) في Sprint 2.
...
```typescript
// src/lib/student-state-machine/mAchine.php
...
كل `on` block يحتوي على `target`, `guard` (من §5.4), و `actions` (من §5.5). التنفيذ الكامل في `src/lib/student-state-machine/mAchine.php` — التفاصيل في `tests/unit/student-state-machine/machine.Test.php` (22 اختبار يغطي كل transition).

### 5.7 Trigger Postgres للحفاظ على الـ History

> **مبدأ الدفاع في العمق:** Spatie Laravel Model States يكتب التاريخ من جانب التطبيق، لكن نضيف **Trigger Postgres** كحارس ثانٍ في حالة تغيير `students.status` يدوياً (`UPDATE` مباشر مثلاً).
...
**`checkPermission(userId, resource, action, scope, targetBranchId?)`** (موقع: `src/lib/rbac/cHeckPermission.php`):
...
**`withPermission` HOF** (موقع: `src/lib/rbac/wIthPermission.php`): تغليف لكل Livewire Action / Laravel Controller — إذا الصلاحية غير موجودة، يرمي `ForbiddenError` (HTTP 403). يستخرج `targetBranchId` من المعطيات إن أمكن (مثلاً من id الطالب → branch_id).

> **استخدام:** كل Livewire Action / Laravel Controller يبدأ بـ `withPermission({ resource: 'students', action: 'update', scope: 'branch' }, async (...) => { ... })`.
...
-- نفعل ذلك في Livewire Action / Laravel Controller مع UPDATE محصور بحقول معيّنة، RLS تسمح
...
- جدول `Filament Tables` بـ pagination (20/50/100) + sorting لكل عمود.
...
**استخدام:**
- Eloquent Query للجلب الأولي عبر Filament Resource / Livewire `mount()`.
- Livewire Component للجدول والـ filters (public properties + `wire:model.live`).
- `<x-loading-skeleton />` Blade Component أثناء انتظار Livewire response.
...
كل تبويب Blade Component مستقل (Streaming + Suspense per tab).
...
كل خطوة Validation بـ Laravel Validation + react-hook-form. الحفظ بعد المراجعة فقط (يُنشئ سجلاً واحداً بحالة `registered`).
...
- عند الضغط: Modal بـ:
  ...
  - زر التأكيد.
- بعد التأكيد:
  - استدعاء Livewire Action / Laravel Controller `changeStudentStatus()`.
  - الـ Action ينفّذ guards، يستدعي Spatie Laravel Model States، يكتب في DB.
...
> **مبدأ:** نستخدم **Livewire Actions / Laravel Controllers** للعمليات الداخلية (UI ↔ Server). REST API endpoints فقط للتكاملات الخارجية أو الـ Bulk operations.

### 9.1 Livewire Actions / Laravel Controllers الأساسية

| الـ Action | الموقع | المدخلات | المخرجات | RBAC |
|-----------|-------|---------|---------|------|
| `getStudentsList(filters, pagination)` | `app/admin/students/aCtions.php` | `{ branchId?, status?, search?, page, pageSize }` | `{ items, total, page }` | حسب المصفوفة |
| `getStudentById(id)` | `app/admin/students/[id]/aCtions.php` | `id: UUID` | `Student` | حسب المصفوفة |
| `createStudent(payload)` | `app/admin/students/new/aCtions.php` | `CreateStudentInput` | `{ id, studentNumber }` | `students:create:branch` |
| `updateStudent(id, payload)` | `app/admin/students/[id]/edit/aCtions.php` | `(id, UpdateStudentInput)` | `Student` | `students:update:*` |
| `changeStudentStatus(id, event, reason)` | `app/admin/students/[id]/change-status/aCtions.php` | `(id, TransitionEvent, reason)` | `Student` (بعد التحديث) | `students:change_status:*` |
| `transferStudent(payload)` | `app/admin/students/[id]/transfer/aCtions.php` | `TransferInput` | `{ transferId }` | `students:transfer_initiate:branch` |
| `approveTransfer(transferId)` | `app/admin/transfers/[id]/aCtions.php` | `transferId: UUID` | `Transfer` | `students:transfer:all` |
| `uploadAttachment(studentId, file, type)` | `app/admin/students/[id]/attachments/aCtions.php` | `(id, FormData, type)` | `Attachment` | `students_attachments:upload:*` |
| `addNote(studentId, payload)` | `app/admin/students/[id]/notes/aCtions.php` | `(id, NoteInput)` | `Note` | `students_notes:create:*` |
| `getStatusHistory(studentId)` | `app/admin/students/[id]/timeline/aCtions.php` | `studentId` | `StatusHistoryEntry[]` | `students_history:read:*` |
| `bulkImportStudents(file)` | `app/admin/students/import/aCtions.php` | `FormData (xlsx)` | `{ jobId }` | `students:import_bulk:all` |
| `getBulkImportStatus(jobId)` | as above | `jobId` | `{ status, processed, total, errors }` | as above |
| `exportStudents(filters, format)` | `app/admin/students/export/aCtions.php` | `(filters, 'xlsx'|'csv')` | `Blob` | `students:export:*` |
| `exportStudentPDF(studentId)` | as above | `studentId` | `Blob (PDF)` | `students_export:own` or branch |
| `getBranchKPIs(branchId?)` | `app/admin/branches/[id]/dashboard/aCtions.php` | `branchId?` | `BranchKPIs` | `branches:view_kpis:*` |
| `updateBranchSettings(id, payload)` | `app/admin/branches/[id]/settings/aCtions.php` | `(id, BranchSettings)` | `Branch` | `branches:update:own/all` |
...
| `POST` | `/api/v1/students/status-cron-tick` | Cron يستدعيه Laravel + PostgreSQL Scheduled Function (PAYMENT_OVERDUE_15D إلخ) | Service Role secret |
| `GET` | `/api/v1/branches/:id/kpis` | KPIs لاستخدام Dashboard خارجي | Bearer Token |

### 9.3 توقيع موحّد للـ Livewire Actions / Laravel Controllers

كل Livewire Action / Laravel Controller يعيد `ActionResult<T>`:
- نجاح: `{ ok: true, data: T }`.
...
sequenceDiagram
    ...
    participant SA as Livewire Action / Laravel Controller
    participant DB as Postgres
    participant ST as Spatie Media Library
    participant AL as Audit Log

    RO->>UI: Wizard 4 خطوات
    UI->>SA: createStudent(payload)
    SA->>SA: checkPermission(students:create:branch)
    SA->>SA: Laravel Validation validate
...
sequenceDiagram
    ...
    participant UI as Frontend
    participant API as Livewire Action / Laravel Controller
    participant DB as Postgres
...
sequenceDiagram
    ...
    participant UI as Frontend
    participant API as Livewire Action / Laravel Controller
    participant FSM as Spatie Laravel Model States
    participant DB as Postgres
...
```
              E2E (Laravel Dusk)
                 ▲
        Integration (Pest + Laravel Sail Local)
                 ▲
            Unit (Pest)
```
...
|------|--------------|---------|
| `student-state-machine/guards.Test.php` | 18 | كل guard + cases الحرجة |
| `student-state-machine/actions.Test.php` | 12 | كل action مع mock للـ DB / Eloquent |
| `student-state-machine/machine.Test.php` | 22 | كل transition + invalid transitions |
| `validators/student.Test.php` | 15 | Form Request + Validation |
| `validators/national-id-checksum.Test.php` | 8 | سعودي 10 أرقام |
| `rbac/check-permission.Test.php` | 14 | كل دور + كل scope |
| `bulk-import/parser.Test.php` | 10 | Excel parsing + Validation |

### 13.3 Integration Tests

اختبارات على Laravel Sail Local Stack:
...
ملف `tests/rls/students-isolation.Spec.php` يستخدم JWT منشأ يدوياً لكل دور. السيناريوهات الإلزامية (12):
...
| **S2: State Machine + Audit** | الأسبوع 2 | Spatie Laravel Model States v5 + Postgres trigger + شاشة Timeline | Machine + tests + شاشة Timeline + Audit |
| **S3: Students CRUD UI** | الأسبوع 3 | قائمة + ملف الطالب + إضافة (Wizard) + تعديل + Storybook | UI Components + Livewire Actions / Laravel Controllers + Storybook + E2E مبدئي |
...
| T# | المهمة | الساعات |
|----|--------|---------|
| S2.1 | تنصيب Spatie Laravel Model States v5 + إعداد types | 2 |
...
| T# | المهمة | الساعات |
|----|--------|---------|
| S3.1 | Livewire Action / Laravel Controller `getStudentsList` مع filters + pagination | 4 |
| S3.2 | شاشة قائمة الطلاب (tanstack-table + filters sidebar) | 8 |
| S3.3 | Livewire Action / Laravel Controller `getStudentById` + شاشة Profile (5 tabs) | 8 |
| S3.4 | شاشة `new` (Wizard 4 steps + Laravel Validation) | 8 |
| S3.5 | شاشة `edit` + Livewire Action / Laravel Controller `updateStudent` | 5 |
| S3.6 | شاشة `change-status` + Livewire Action / Laravel Controller مع Spatie Laravel Model States | 5 |
...
| S4.2 | HOF `withPermission()` لكل Livewire Actions / Laravel Controllers | 4 |
...
| S5.7 | Livewire Action / Laravel Controller Export Excel | 4 |
...
| S6.2 | E2E Laravel Dusk (10 سيناريو) | 10 |
...
1. ✅ الكود مكتوب وفق معايير §16 (PHP 8.3 strict + Laravel Pint + PHPStan/Larastan).
2. ✅ Unit tests تغطّي ≥ 80% من الـ code paths المنطقية.
3. ✅ Integration test على Laravel Sail Local يمر.
...
| Livewire Actions / Laravel Controllers + Validation | 30 |
...
- ✅ كل Livewire Action / Laravel Controller تستدعي `withPermission`.
...
| **R3:** State Machine يصبح معقّداً عند إضافة حالات | صعوبة الصيانة | منخفضة | استخدام Spatie Laravel Model States `setup()` typed APIs + توثيق كامل في README |
...
| **R6:** Spatie Media Library policies غير محكَمة | تسرب مرفقات | متوسطة | اختبار صريح لـ Storage RLS + signed URLs مع TTL |
...
| **R9:** الـ employment_info JSON غير منظّم | بيانات قذرة | متوسطة | Form Request + Validation للـ JSON عند الإدخال + DB CHECK constraint على الـ keys |
...
src/lib/pdf/
└── student-profile.blade.php (Livewire view)

src/components/students/
├── students-table.blade.php (Livewire view)
├── student-profile-tabs.blade.php (Livewire view)
├── student-form-wizard.blade.php (Livewire view)
├── status-change-modal.blade.php (Livewire view)
├── transfer-modal.blade.php (Livewire view)
├── attachment-uploader.blade.php (Livewire view)
├── notes-section.blade.php (Livewire view)
└── timeline-view.blade.php (Livewire view)

src/components/branches/
├── branch-card.blade.php (Livewire view)
├── branch-kpis.blade.php (Livewire view)
├── branch-settings-form.blade.php (Livewire view)
└── working-hours-editor.blade.php (Livewire view)

app/Filament/Admin/Resources/StudentResource.php / Pages/
├── page.blade.php (Livewire view)
├── new/page.blade.php (Livewire view)
├── new/aCtions.php
├── import/page.blade.php (Livewire view)
├── import/aCtions.php
├── export/aCtions.php
└── [id]/
    ├── page.blade.php (Livewire view)
    ├── edit/page.blade.php (Livewire view)
    ├── edit/aCtions.php
    ├── transfer/page.blade.php (Livewire view)
    ├── transfer/aCtions.php
    ├── change-status/page.blade.php (Livewire view)
    ├── change-status/aCtions.php
    ├── timeline/page.blade.php (Livewire view)
    ├── audit/page.blade.php (Livewire view)
    └── attachments/page.blade.php (Livewire view)

app/Filament/Admin/Resources/BranchResource.php / Pages/
├── page.blade.php (Livewire view)
└── [id]/
    ├── dashboard/page.blade.php (Livewire view)
    └── settings/page.blade.php (Livewire view)

app/Filament/Admin/Resources/TransferResource.php / Pages/
├── page.blade.php (Livewire view)
└── [id]/aCtions.php
...
tests/integration/
├── students-crud.Test.php
├── bulk-import.Test.php
└── transfers.Test.php

tests/rls/
├── students-isolation.Test.php
├── branches-isolation.Test.php
└── attachments-isolation.Test.php

tests/e2e/
├── student-registration.Spec.php
├── student-transfer.Spec.php
├── status-change.Spec.php
└── bulk-import.Spec.php
...
## ملحق ب: قائمة الحزم (Composer + NPM)

```json
// composer.json — Laravel packages
{
  "require": {
