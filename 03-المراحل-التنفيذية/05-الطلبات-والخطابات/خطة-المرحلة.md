...
1. **محرك الطلبات (Workflow Engine):** 14 نوع طلب × 7 حالات × توجيه ذكي بين الأقسام × SLA مع تصعيد تلقائي. State Machine قوي مبني على Spatie Laravel Model States يمنع الأخطاء البشرية.
...
| المخاطرة | الردّ |
|---------|------|
| Browsershot (spatie/browsershot) ثقيل على Serverless (Laravel Controllers) | استخدام `@sparticuz/chromium` + Cold-Start mitigation عبر Warmup Cron. خيار B: Edge Function منفصل على VPS سعودي للخطابات الكثيفة. |
| تعقيد State Machine ينمو بسرعة | Spatie Laravel Model States مع **machine واحد قاعدي** + 14 overrides. اختبارات Property-Based تغطي كل الانتقالات. |
...
✅ Workflow Engine للطلبات (14 نوع، 7 حالات، Spatie Laravel Model States).
✅ Routing Engine قابل للتخصيص (افتراضي + يدوي).
✅ SLA Engine مع Business Hours + Escalation Cron.
✅ Letters Generator (محرك القوالب + Browsershot (spatie/browsershot)).
...
❌ **توقيع إلكتروني معتمد قانونياً (OTP/SMS / Saudi Trust)** — مؤجل. التوقيع الرقمي الحالي = SHA-256 + QR للتحقق فقط.
...
|--------|--------|------|
| `spatie/laravel-model-states` | ^2.7 | State Machine للطلبات والتذاكر |
| `filament/filament` | ^3.2 | Resources/Forms/Tables للطلبات والتذاكر |
| `livewire/livewire` | ^3.5 | Reactive components للنماذج الديناميكية |
| `spatie/browsershot` | ^4.0 | تحويل HTML → PDF (Chromium محلي) |
| `endroid/qr-code` | ^5.0 | توليد QR للتحقق |
| `bacon/bacon-qr-code` | ^3.0 | بديل لتوليد QR (داخل Filament Actions) |
| `nesbot/carbon` + `genealabs/laravel-pivot-events` | ^3.0 / ^11.0 | تواريخ Hijri/Gregorian + Business Hours |
| `laravel/horizon` + Laravel Scheduler | ^5.0 | Escalation Jobs (queue + cron عبر `artisan schedule:run`) |
| `spatie/laravel-data` + Form Requests | ^4.0 | DTOs + Validation للنماذج الديناميكية |
| `spatie/laravel-activitylog` | ^4.8 | Audit log + Versioning للطلبات |

#### 2.1.2 الـ Modules

| الـ Module | المسار | المسؤولية |
|-----------|--------|----------|
| `workflow-engine` | `src/modules/workflow-engine/` | تشغيل Spatie Laravel Model States + Routing + SLA |
| `letters-generator` | `src/modules/letters-generator/` | القوالب + Browsershot (spatie/browsershot) + التوقيع |
| `cv-generator` | `src/modules/cv-generator/` | بناء PDF السيرة الذاتية |
| `help-desk` | `src/modules/help-desk/` | التذاكر الداخلية |
| `requests-portal` | `resources/views/livewire/student/requests/` + `app/Livewire/Student/Requests/` | شاشات الطالب |
| `requests-staff` | `resources/views/livewire/staff/requests/` + `app/Livewire/Staff/Requests/` | شاشات الموظف |
| `requests-admin` | `resources/views/livewire/admin/requests/` + `app/Livewire/Admin/Requests/` | لوحة الإدارة |
| `verify-public` | `resources/views/letters/verify.blade.php` + `app/Http/Controllers/LetterVerifyController.php` | صفحة التحقق العامة |
...
| `letters-storage-cleanup` | أسبوعياً | فحص تطابق DB مع Spatie Media Library |
...
| Deployment Notes (Browsershot (spatie/browsershot)) | `docs/deployment/puppeteer.md` |
...
- 14 Spatie Laravel Model States Machine لكل نوع طلب.
- 4 Spatie Laravel Model States Machines لأنواع التذاكر الداخلية.
- 8 شاشات للطالب + 9 شاشات للموظف + 5 شاشات للإدارة.
- 38 API Route (Laravel Controllers).
...
الحالة الحقيقية للطلب في **PostgreSQL** (`requests.status`). Spatie Laravel Model States يُستخدم للـ **Validation + Routing** فقط، لا يُحتفظ بحالة في الذاكرة. عند كل تحوّل: نُحدث DB → ثم نُحدث الـ UI من DB.

#### مبدأ 2 — Server-Authoritative
كل تحوّل حالة يُوقَّع على السيرفر (Laravel Controller) عبر:
1. فحص الصلاحية (Permission Check).
2. تشغيل Spatie Laravel Model States Validation.
...
│         (Laravel Pages + Blade Components + Forms)            │
└─────────────┬───────────────────────────────────────────────────┘
              │
              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│  │   (Spatie Laravel Model States)     │  │  (Browsershot (spatie/browsershot))    │  │  (Tickets)     │   │
│  └────────┬───────┘  └────────┬────────┘  └────────┬───────┘   │
│           │                   │                    │            │
│  ┌────────▼───────────────────▼────────────────────▼───────┐  │
...
└─────────────────────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│           Laravel + PostgreSQL (PostgreSQL + Storage + Auth)               │
└─────────────────────────────────────────────────────────────────┘
```
...
flowchart TB
    ...
    subgraph "Workflow Core"
        WE[Workflow Engine - Spatie Laravel Model States]
        SLA[SLA Calculator]
        RT[Routing Engine]
    end
...
#### ADR-501: Spatie Laravel Model States 5 بدلاً من State Machine يدوي
**القرار:** نستخدم Spatie Laravel Model States 5 كمحرك الـ State Machine.
**البديل المرفوض:** كتابة State Machine يدوي بـ PHP 8.3 + Map.
**الأسباب:**
- Spatie Laravel Model States يوفر **Statecharts** (hierarchical + parallel states) — نحتاجها لـ Help Desk.
- Visualization عبر Laravel Telescope + Spatie ModelStates events يوفر debugging قوي.
- Test utilities جاهزة (Model-Based Testing).
- مجتمع كبير + موثوق.
**التكلفة:** ~80KB إضافية (Bundle مقبول).

#### ADR-502: Browsershot (spatie/browsershot) بدلاً من PDFKit / jsPDF
**القرار:** نستخدم Browsershot (spatie/browsershot) + `@sparticuz/chromium`.
**البدائل المرفوضة:**
...
**التكلفة:** Cold start ~3-5 ثوانٍ على VPS KSA (Hostinger / Contabo). الحل: Warmup Cron + Edge Function منفصل.
...
- يتشارك في Spatie Laravel Model States Machine الأساسية فقط.
...
**الأسباب:**
- PKI معتمد قانونياً = OTP/SMS / Saudi Trust — مؤجل للمرحلة 8.
...
```typescript
// src/modules/workflow-engine/sla/cAlculator.php
...
```typescript
// src/modules/workflow-engine/sla/eScalationCron.php
// Laravel: استعمل query بـ Service Role (بدون Policy Scope)
...
export async function runEscalationTick(): Promise<{ checked: number; escalated: number }> {
  ...
  const { data: overdue, error } = await dbServiceRole
    .from('requests')
  ...
  for (const req of overdue) {
    ...
    // نفّذ التصعيد
    await dbServiceRole.rpc('escalate_request', {
      p_request_id: req.id,
...
### 4.5 Spatie Laravel Model States Implementation الكامل

```php
// app/Models/States/Request/BaseRequestState.php
...
```typescript
// app/Http/Controllers/RequestTransitionController.php + routes/api.php
import { NextRequest, NextResponse } from 'laravel/server';
// Laravel Controller - استعمل Eloquent Models و DB Facade
...
CREATE TABLE request_attachments (
    ...
    storage_path    TEXT NOT NULL,    -- media library disk path
...
sequenceDiagram
    ...
    participant CH as Chromium (Browsershot (spatie/browsershot))
...
```typescript
// src/modules/letters-generator/iNdex.php
...
export class LettersGenerator {
  constructor(
    ...
    private storage: MediaLibraryService,
  ...
  async issue(input: IssueLetterInput): Promise<IssuedLetter> {
    ...
    // 11. Persist
    const { data: letter } = await $this->db
      .from('letters')
      .insert({
        serial,
  ...
  private async launchChromium() {
    if (env('APP_ENV') === 'production' && env('CHROMIUM_REMOTE_URL')) {
...
```typescript
// app/Http/Controllers/letters/verify/[serial]/page.blade.php (Livewire view)
import { notFound } from 'laravel/navigation';
// Laravel public route - Eloquent بدون auth context
import { headers } from 'laravel/headers';
...
export default async function VerifyLetterPage({ params, searchParams }: PageProps) {
  ...
  await DB::table('letter_verifications').insert({
...
```typescript
// src/modules/cv-generator/iNdex.php
...
sequenceDiagram
    ...
    participant WE as Workflow Engine (Spatie Laravel Model States)
    ...
    participant LG as Letters Generator
    participant PUP as Browsershot (spatie/browsershot)
    participant ST as Spatie Media Library
...
## 10. API Routes الكاملة (Laravel Controllers)
...
| E2E Tests (Laravel Dusk) | ~25 | الـ Happy Paths لكل نوع طلب رئيسي |
...
```typescript
// tests/integration/request-lifecycle.Test.php
...
```typescript
// tests/visual/templates.Test.php
...
### 12.3 Property-Based Testing للـ Spatie Laravel Model States

```php
// tests/Feature/Workflow/RequestStateMachineTest.php
use Pest\Faker\fake;
use App\Models\StudentRequest;
use App\Models\States\Request\{Pending, UnderReview, Approved, Rejected};
...
```typescript
// tests/load/letters-throughput.K6.php
...
- Spatie Media Library Bucket `letters` بـ `private` mode.
...
| إصدار خطاب (Browsershot (spatie/browsershot)) | 1.8s | 4s | 7s |
...
#### 14.2.1 Browsershot (spatie/browsershot) Cold-Start Mitigation
...
|---------|----------------|---------|
| **أسبوع 1** | Migrations + Routing + SLA Engine + Spatie Laravel Model States base machine | DB schema جاهز، Workflow Engine يعمل أساسياً |
...
|---------|---------|--------|---------|
| Browsershot (spatie/browsershot) cold-start مزعج | متوسط | متوسط | Warmup cron + Edge function للخطابات الكثيفة |
...
- [ ] كل الـ Migrations في `database/migrations/` ومُختبرة على بيئة dev.
...
5. **توقيع OTP/SMS معتمد** — في المرحلة 8.
...
│   ├── workflow-engine/
│   │   ├── machines/
│   │   │   ├── base-request.Machine.php
│   │   │   ├── ticket.Machine.php
│   │   │   └── overrides/{14 files}.php
│   │   ├── sla/
│   │   │   ├── Calculator.php
│   │   │   ├── EscalationCron.php
│   │   │   └── WorkingCalendar.php
...
│   │   ├── Generator.php
│   │   ├── TemplateEngine.php
│   │   ├── BrowsershotLauncher.php (spatie/browsershot)
│   │   ├── QrBuilder.php
│   │   ├── HashUtils.php
│   │   ├── Storage.php
│   │   └── Index.php
...
### D. مرجع الأنواع PHP 8.3 الرئيسية
...
4. **التوقيع الرقمي:** هل سيكتفي العميل بـ SHA-256 + QR؟ أم يريد PKI معتمد (يفتح نقاش مع OTP/SMS في المرحلة 8)؟
