...
| Supabase + Postgres + RLS | المرحلة 0 | ✅ جاهز |
...
1. **عزل تام (Strong Isolation):** أي ملف في `src/server/integrations/accounting/` لا يستورد إلا من `types.ts`. لا تسرّب نوع `AxiosResponse` خارج الـAdapter.
...
```
src/server/integrations/accounting/
├── types.ts                  # كل الـtypes — مصدر الحقيقة
├── interface.ts              # IAccountingService
├── adapters/
│   ├── mock.ts               # MockAccountingAdapter — للتطوير
│   ├── external.ts           # ExternalAccountingAdapter — للإنتاج
│   ├── onyx.ts               # OnyxAccountingAdapter (اختياري)
│   └── daftra.ts             # DaftraAccountingAdapter (اختياري)
├── http-client.ts            # axios + interceptors
├── resilience.ts             # CircuitBreaker + Retry + DLQ
├── cache.ts                  # طبقة Caching على Postgres
├── webhook-receiver.ts       # POST /webhooks/accounting
├── webhook-verifier.ts       # HMAC verification
├── sync-worker.ts            # Pull periodic
├── reconciliation.ts         # Daily reconciliation
└── di.ts                     # Dependency injection container
```

#### 4.1.2 الـ Contract المتوقَّع (TypeScript Interfaces)
...
```typescript
// src/server/integrations/accounting/interface.ts
...
**التطبيق:**
- API Key يُحفَظ في **Supabase Vault** أو **environment variable** مشفّر.
...
```typescript
// src/server/integrations/accounting/http-client.ts
...
```typescript
// src/server/integrations/accounting/adapters/external.ts
...
```typescript
// src/server/integrations/accounting/adapters/mock.ts
...
```typescript
// src/server/integrations/accounting/di.ts
...
نستخدم **Supabase pg_cron + pg_net** (دون Edge Function منفصل):
...
```typescript
// src/app/api/webhooks/accounting/route.ts
export async function POST(req: Request) {
...
```typescript
// src/app/api/cron/accounting-reconciliation/route.ts
export async function POST(req: Request) {
  if (req.headers.get('x-cron-secret') !== process.env.CRON_SECRET) {
    return new Response('Unauthorized', { status: 401 });
  }
...
- **التشفير في التخزين:** Supabase AES-256 + pgcrypto للحقول الحسّاسة.
...
1. **Authentication:** كل route محمي بـSupabase Auth + JWT.
...
4. **API Key Storage:** في Supabase Vault، لا في كود ولا في `.env` غير مشفّر.
...
- [x] التشفير في التخزين (Supabase AES-256 + pgcrypto للحساس).
...
- [ ] جميع ملفات `src/server/integrations/accounting/` معزولة.
...
- [ ] README في `src/server/integrations/accounting/`.
