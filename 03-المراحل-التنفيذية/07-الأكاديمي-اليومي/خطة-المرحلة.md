...
هذه المرحلة هي **أكبر مرحلة وظيفية** في المشروع كله من حيث عدد الجداول الجديدة (≈ 15 جدول) وعدد الشاشات (≈ 30 شاشة) وعدد الـ Workflows (≈ 12 workflow) وعدد الـ Laravel Jobs / Scheduled Tasks (≈ 8 functions). كل تطوير في النظام بعدها — التقارير، الأرشيف، التكامل مع TVTC، الذكاء الاصطناعي — سيستند على ما نُنتجه هنا.
...
| D2 | API كامل لإدارة الجداول | `app/Services/actions/schedules/` | منع تعارضات على 3 مستويات (قاعة/معلم/طالب) |
...
| D11 | Laravel Jobs / Scheduled Tasks | `app/Jobs/Scheduled/{eligibility,warnings,calendar-export,response-time}` | 4 functions تعمل في الإنتاج |
| D12 | Background Jobs (pg_cron) | `cron.schedule(...)` | 5 جدولات (warning-check، reminder، archive، etc.) |
| D13 | E2E Tests | `e2e/phase7/*.Spec.php` | 25 سيناريو تغطي كل التدفقات الحرجة |
...
| `notifications` table + Realtime channel | المرحلة 1 | يمكن إرسال إشعار من Laravel Job / Scheduled Task |
...
| `pg_cron` (Laravel + PostgreSQL Extension) | جدولة الـ Background Jobs | تفعيل في Filament Admin Panel |
...
- **Laravel Service Classes / Jobs Cold Start optimization** — للـ Eligibility Engine.
- **Blade Components + Livewire 3 + Suspense Streaming** — لـ Calendar اللي قد يعرض 200 حدث.
...
#### 4.1.4 الـ APIs / Livewire Actions / Laravel Controllers

```typescript
// app/Services/actions/academic-terms/cReateTerm.php
'use server';
...
```typescript
// app/Jobs/Scheduled/attendance-warnings/iNdex.php
...
#### 4.7.4 Eligibility Engine — PHP 8.3

```typescript
// app/Services/eligibility/cOmprehensiveExam.php
...
#### 4.8.4 Blade/Livewire Component (مختصر)

```php
<?php
// app/Livewire/Calendar/UnifiedCalendar.php
...
class UnifiedCalendar extends Component
{
    ...
    public string $role;
    public string $view = 'week';           // month | week | day
    public array $filters = ['session', 'assignment', 'exam', 'invoice'];
    ...
    #[Computed]
    public function visibleEvents(): array
    {
        return app(CalendarService::class)
            ->fetchEvents(userId: $this->userId, role: $this->role)
            ->filter(fn ($e) => in_array($e->type, $this->filters))
            ->all();
    }
    ...
    public function toggleFilter(string $type): void
    {
        if (in_array($type, $this->filters)) {
            $this->filters = array_diff($this->filters, [$type]);
        } else {
            $this->filters[] = $type;
        }
    }
    ...
    {
        return view('livewire.calendar.unified-calendar');
    }
}
```

```blade
{{-- resources/views/livewire/calendar/unified-calendar.blade.php --}}
<div class="flex flex-col gap-3" dir="rtl">
    <header class="flex justify-between items-center">
        <x-calendar.view-switcher :view="$view" />
        <x-calendar.filter-bar :filters="$filters" />
        <x-calendar.export-button :user-id="$userId" />
    </header>

    @switch($view)
        @case('month')  <livewire:calendar.month-view :events="$this->visibleEvents" />  @break
        @case('week')   <livewire:calendar.week-view  :events="$this->visibleEvents" />  @break
        @case('day')    <livewire:calendar.day-view   :events="$this->visibleEvents" />  @break
    @endswitch

    <x-calendar.event-legend />
</div>
```
...
{
    public function __invoke(Request $request, string $token, CalendarService $calendar, IcsBuilder $ics): Response
    {
        $userId = $calendar->verifyToken($token);

        if (! $userId) {
            return response('Forbidden', 403);
        }

        $events = $calendar->fetchEvents(userId: $userId);

        $icsContent = $ics->build($events->map(fn ($e) => [
            'start'       => $e->start,
            'duration'    => $e->durationHours(),
            'title'       => $e->title,
            'description' => $e->description ?? '',
            'location'    => $e->location ?? '',
            'uid'         => "{$e->type}-{$e->id}@iims.sa",
            'categories'  => [$e->type],
        ]));
...
// routes/api.php:
// Route::get('/calendar/{token}/feed.ics', CalendarFeedController::class);
```
...
graph TB
    ...
    Comments --> Realtime[Laravel Reverb (WebSockets)]
```
...
SELECT cron.schedule('attendance-warnings', '0 * * * *',
  $$ SELECT net.http_post('https://<project>.iims-staging.example.sa/functions/v1/attendance-warnings',
...
- OTP/SMS Authentication (المرحلة 9).
...
- **ملفات الواجبات والمكتبة**: مشفّرة at-rest (Spatie Media Library default + Server-Side AES-256).
...
|--------|------|-------------------|
| Unit | Pest | ≥ 80% للـ Eligibility Engine + Late Penalty + Warning Levels |
| Integration | Pest + Test DB | كل Livewire Actions / Laravel Controllers + Laravel Jobs / Scheduled Tasks |
| E2E | Laravel Dusk | 25 سيناريو رئيسي |
| Load | k6 | 1000 طالب × eligibility في < 60s |
| Offline | Laravel Dusk + Network Throttling | Sync بعد قطع شبكة |
...
- محاولة XSS في وصف المورد → escaped في Livewire/Blade.
...
| 3 | Livewire Actions / Laravel Controllers للـ Terms + Holidays | Senior | tests نجاح |
| 4 | Livewire Actions / Laravel Controllers للـ Rooms + Time Slots | Senior | + Seeds |
| 5 | Livewire Actions / Laravel Controllers للـ Courses + Schedules | Senior | + منع تعارض المعلم |
...
| 12 | Livewire Actions / Laravel Controllers للحضور + Bulk Mark + Edit | Senior | يعمل في 100ms |
...
| 15 | Livewire Actions / Laravel Controllers للأعذار + الموافقة | Mid | + رفع PDF |
...
| 19 | Livewire Actions / Laravel Controllers للواجبات + Late Penalty engine | Senior | + tests للسياسات الثلاث |
...
| 22 | Eligibility Engine PHP 8.3 + Laravel Job / Scheduled Task | Senior | 1000 طالب < 60s |
...
| 26 | Blade/Livewire Component `<UnifiedCalendar />` + Hooks | Senior | 3 views + Filters |
...
| Livewire Actions / Laravel Controllers + Laravel Jobs / Scheduled Tasks | 80h | 21% |
...
- [ ] 4 Laravel Jobs / Scheduled Tasks: `attendance-warnings`, `eligibility-engine`, `calendar-feed`, `notify-batch`.
- [ ] 5 pg_cron jobs مُجدوَلة.
- [ ] 30 شاشة UI كاملة (Desktop + Mobile).
- [ ] Blade/Livewire Component `<UnifiedCalendar />` معاد استخدامه.
- [ ] Blade/Livewire Component `<LibraryGrid />` معاد استخدامه.
- [ ] Hook `useCalendarEvents` + `useAttendanceSync` + `useLibrarySearch`.
- [ ] PHP 8.3 types مولّدة من DB schema.
- [ ] كل Livewire Action / Laravel Controller مغطّى بـ Validation Schema (Laravel Validation).
...
- [ ] Integration tests لكل Livewire Action / Laravel Controller.
- [ ] E2E: 25 سيناريو يعمل.
...
- [ ] `docs/phase7/api.md` — كل Livewire Action / Laravel Controller مع شرحه.
...
- **`pg_cron` على PostgreSQL على VPS KSA** يستدعي Laravel Jobs / Scheduled Tasks عبر `net.http_post` لا داخل SQL.
- **Materialized Views** على Laravel + PostgreSQL تحتاج REFRESH يدوي عبر cron — لا تتحدث تلقائياً.
...
- **PDF Generation** للمرة 1000 يستهلك ذاكرة — استعمل Edge Function منفصلة + Stream، لا Livewire Action / Laravel Controller.

### 16.3 ما لا تفعله

- ❌ لا تربط الـ Calendar مباشرة بـ Realtime لكل event — مكلف. استعمل polling كل 60s + invalidate on user action.
- ❌ لا تخزّن فيديوهات الواجبات في Spatie Media Library (مكلف). اطلب من الطالب رفع لـ YouTube/Google Drive + لصق الرابط.
...
- ✅ كل Livewire Action / Laravel Controller له test + Form Request + Validation + audit_log.
