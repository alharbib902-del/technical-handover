...
- ❌ **الربط مع وزارة الموارد البشرية أو نظام OTP/SMS** — مؤجَّل (قرار العميل: لاحقاً).
...
- **150+ Unit Tests + 25 E2E Tests** (Laravel Dusk).
...
|------|----------------------|------------|
| Laravel 11 + PHP 8.3 Project مُهيَّأ | DevOps Lead | `php artisan serve` يعمل بلا أخطاء |
| مشروع Laravel + PostgreSQL موجود + متّصل | Tech Lead | RLS مفعّل + Migrations stack جاهز |
| جداول Core (`tenants`, `branches`, `users`, `roles`, `audit_log`, `students`) موجودة | DBA | فحص Schema يدوي |
| Auth Flow كامل (login, logout, password reset, 2FA لـsuper_admin) | Auth Lead | اختبار يدوي + Laravel Dusk |
...
| Tailwind + Filament 3 UI مُهيَّأ + Theme عربي RTL | UI Lead | لقطة شاشة من Login |
| Audit Log Trigger يعمل على Core Tables | DBA | INSERT/UPDATE يُسجَّل تلقائياً |
| Spatie Media Library Buckets جاهزة: `attachments`, `letters`, `certificates`, `media-questions` | DevOps | فحص في Filament Admin Panel |
| Laravel Pint + PHPStan/Larastan + Lefthook / Git Hooks pre-commit hooks | Tech Lead | `git commit` يفشل عند Warning |
| CI Pipeline (GitHub Actions) — Build + Test + Lint | DevOps | آخر Build في `main` Green |
| `.env.example` كامل + Secret Management عبر VPS KSA (Hostinger / Contabo)/Laravel + PostgreSQL | DevOps | لا أسرار في الـrepo |
...
required_services:
  - name: Laravel + PostgreSQL
    ...
    features: [Postgres, Auth, Storage, Realtime, Laravel Jobs / Scheduled Tasks]
  - name: VPS KSA (Hostinger / Contabo)
    plan: Pro (للـ Cron Jobs و Background Functions)
    features: [Laravel Jobs / Scheduled Tasks, Cron, Analytics]
...
required_libraries:
  # Composer (PHP 8.3 / Laravel 11)
  - "laravel/framework": "^11.0"
  - "filament/filament": "^3.2"
  - "livewire/livewire": "^3.5"
  - "spatie/laravel-model-states": "^2.7"     # State Machine للمحاولات
  - "spatie/browsershot": "^4.0"               # توليد PDF عبر Chromium
  - "bacon/bacon-qr-code": "^3.0"              # QR codes للشهادات
  - "maatwebsite/excel": "^3.1"                # استيراد/تصدير Excel
  - "anthropic-ai/sdk": "^0.5"                 # Claude AI (أو HTTP Client)
  # NPM (assets فقط عبر Vite)
  - "tailwindcss": "^3.4"
  - "alpinejs": "^3.13"
  - "katex": "^0.16.11"                        # عرض LaTeX
  - "mathjs": "^13.0.0"                        # تقييم رياضي
  - "@hello-pangea/dnd": "^16.6.0"             # Drag & Drop questions (داخل Livewire/Alpine)
  - "@anthropic-ai/sdk": "^0.27.0"             # (اختياري للـ JS code question runner)
```
...
|--------|------|---------|---------------|
| **يدوي** | Form ديناميكي يتغيّر حسب النوع | Form Request + Validation lifecycle (Client → Server) | تحقق لحظي + رسائل خطأ عربية |
| **Excel Import** | Drag & Drop + قالب جاهز للتحميل | exceljs → Laravel Validation array | تقرير أخطاء صف بصف + Undo قبل الحفظ |
...
```typescript
// lib/grading/nOrmalizeArabic.php
...
#### 4.3.3 إعدادات الاختبار (Exam Settings) — PHP 8.3
...
```typescript
// app/api/exams/[id]/regenerate-code/rOute.php
...
- `/exam/[id]/projector` — Full-screen، الكود بخط 96pt، QR، عدّاد الداخلين Live (Laravel Reverb (WebSockets)).
...
```typescript
// app/api/grading/[attemptId]/approve/rOute.php
...
- **بدون تسجيل دخول.**
- **Rate Limiting:** 30 طلب/IP/دقيقة (عبر Redis (Laravel) في Edge Middleware).
...
```typescript
// app/Http/Controllers/LetterVerifyController.php (public route)
...
```typescript
// lib/integrations/tvtc/aDapter.php
...
const config = withPWA({
  ...
  runtimeCaching: [
    {
      urlPattern: /^https:\/\/.*\.db\.co\/.*$/,
      handler: 'NetworkFirst',
      options: {
        cacheName: 'eloquent-cache',
...
```php
<?php
// app/Livewire/Attendance/MobileAttendance.php
...
class MobileAttendance extends Component
{
    public string $sessionId;
    public array $students = [];        // Livewire reactive state (Student list)
    public array $pending = [];         // Livewire reactive state (AttendanceRecord queue)

    public function mount(string $sessionId): void
    {
        $this->sessionId = $sessionId;
        $this->students = Session::with('students')
            ->findOrFail($sessionId)
            ->students
            ->toArray();
    }

    // ينفّذ عبر Livewire wire:click من الـ Blade
    public function markAttendance(string $studentId, string $status): void
    {
        ...
        // أولوية: حفظ مبدئي على الجهاز عبر Alpine.js + IndexedDB (انظر الـ Blade)
        $this->pending[] = $record;

        // محاولة الكتابة الفورية للـ DB (تحت Job مع retry تلقائي)
        AttendanceRecord::create($record);

        // إزالة من قائمة الـ pending
        $this->pending = array_filter($this->pending, fn ($r) => $r['student_id'] !== $studentId);
    }

    #[On('echo:attendance,SyncCompleted')]      // Laravel Reverb
    public function onSyncCompleted(array $payload): void
    {
        // تحديث الواجهة عند مزامنة pending offline records
        $this->pending = array_filter(
            $this->pending,
            fn ($r) => ! in_array($r['student_id'], $payload['synced_ids'])
        );
    }

    public function render()
    {
        return view('livewire.attendance.mobile-attendance');
    }
}
```

```blade
{{-- resources/views/livewire/attendance/mobile-attendance.blade.php --}}
<div class="p-4" x-data="offlineAttendance({{ Js::from($pending) }})">
    {{-- Banner عند انقطاع الإنترنت --}}
    <template x-if="!navigator.onLine">
        <x-offline-banner :pending-count="$pending->count()" />
    </template>
...
@script
<script>
    // Alpine.js للـ IndexedDB offline cache
    Alpine.data('offlineAttendance', (initialPending) => ({
        pending: initialPending,
        async init() {
            window.addEventListener('online', () => this.syncPending());
        },
        async syncPending() {
            // مزامنة سجلات IndexedDB → Server عبر axios
            const cached = await idb.getAll('pending_attendance');
            await axios.post('/api/attendance/sync', { records: cached });
            await idb.clear('pending_attendance');
        }
    }));
</script>
@endscript
...
| 🔴 **Advanced** | كل ما سبق + Realtime monitoring (Laravel Reverb (WebSockets)) للمعلم | الاختبارات الحساسة |

#### 4.14.2 تطبيق Tier 1 (Basic) — على كل المحاولات

```php
<?php
// app/Livewire/Exam/AttemptForm.php — Livewire يحفظ تلقائياً كل 20ث عبر wire:poll
namespace App\Livewire\Exam;

use App\Models\ExamAttempt;
use Livewire\Component;

class AttemptForm extends Component
{
    ...
    {
        ExamAttempt::find($this->attemptId)
            ?->update(['draft_answers' => $this->currentAnswers]);
    }

    public function render()
    {
        return view('livewire.exam.attempt-form');
    }
}
```

```blade
{{-- resources/views/livewire/exam/attempt-form.blade.php --}}
{{-- wire:poll يستدعي saveDraft تلقائياً كل 20 ثانية --}}
<div wire:poll.20s="saveDraft">
    {{-- أسئلة الاختبار --}}
    @foreach ($questions as $q)
        <x-exam.question :question="$q" wire:model.live.debounce.500ms="currentAnswers.{{ $q->id }}" />
    @endforeach
</div>
```
...
```php
<?php
// app/Livewire/Exam/SecurityWrapper.php — Livewire Component يدير حالة الأمان
namespace App\Livewire\Exam;

use Livewire\Component;
use Livewire\Attributes\On;
use App\Models\ExamAttempt;
use App\Models\AttemptSecurityEvent;
use App\Jobs\ForceSubmitAttempt;

class SecurityWrapper extends Component
{
    public string $attemptId;
    public array $options;              // SecurityOptions
    public int $tabBlurCount = 0;

    /** ينفّذ من Alpine.js عبر $wire.recordSecurityEvent */
    public function recordSecurityEvent(string $kind): void
    {
        AttemptSecurityEvent::create([
        ...
        if ($kind === 'tab_blur') {
            ...
            if ($this->tabBlurCount >= 3) {
                $this->dispatch('toast', type: 'warning', message: 'تنبيه: تم رصد تبديل التابات. التكرار قد يؤدي لإنهاء الاختبار.');
            }
            if ($this->tabBlurCount >= 5) {
                ForceSubmitAttempt::dispatch($this->attemptId);
                $this->redirect("/exam/{$this->attemptId}/result");
            }
        }
    }
    ...
    public function render()
    {
        return view('livewire.exam.security-wrapper');
    }
}
```

```blade
{{-- resources/views/livewire/exam/security-wrapper.blade.php --}}
<div x-data="examSecurity({{ Js::from($options) }})" x-init="init()">
    {{ $slot }}
</div>

@script
<script>
    Alpine.data('examSecurity', (options) => ({
        init() {
            if (options.detect_tab_switch) {
                window.addEventListener('blur', () =>
                    $wire.recordSecurityEvent('tab_blur')
                );
            }
            ...
            if (options.require_fullscreen) {
                ...
                document.addEventListener('fullscreenchange', () => {
                    if (!document.fullscreenElement) {
                        $wire.recordSecurityEvent('fullscreen_exit');
                    }
                });
            }
...
class ExamMonitor extends Component
{
    ...
    public function mount(string $examId): void
    {
        $this->examId = $examId;
        $this->loadLiveAttempts();
    }

    public function loadLiveAttempts(): void
    {
        $this->liveAttempts = ExamAttempt::where('exam_id', $this->examId)
            ->where('status', 'in_progress')
            ->with(['student', 'securityEvents'])
            ->get();
    }

    /** يُستمَع له من Laravel Reverb broadcast — channel: exam.{id}.monitor */
    #[On('echo:exam.{examId}.monitor,ExamAttemptUpdated')]
    public function onAttemptUpdated($payload): void
    {
        $this->loadLiveAttempts();
    }

    #[On('echo:exam.{examId}.monitor,SecurityEvent')]
    public function onSecurityEvent($payload): void
    {
        $this->dispatch('toast',
            type: 'warning',
            message: "تنبيه أمني: طالب {$payload['student_name']} — {$payload['kind']}"
        );
        $this->loadLiveAttempts();
    }
    ...
    public function render()
    {
        return view('livewire.teacher.exam-monitor');
    }
}
```
...
<div>
    <h1>مراقبة حية: {{ count($liveAttempts) }} طالب نشط</h1>
    <div class="grid grid-cols-3 gap-4">
        @foreach ($liveAttempts as $attempt)
            <x-teacher.attempt-card
                :attempt="$attempt"
                :alert-level="$attempt->alertLevel()"
            />
        @endforeach
    </div>
</div>
...
```typescript
// components/exam/
├── question-preview.blade.php (+ Livewire class)          // معاينة سؤال بأي نوع
├── question-editor.blade.php (+ Livewire class)           // محرر ديناميكي (يفتح المحرر المناسب)
├── mcq-single-editor.blade.php (+ Livewire class)
├── mcq-multi-editor.blade.php (+ Livewire class)
├── true-false-editor.blade.php (+ Livewire class)
├── fill-blank-editor.blade.php (+ Livewire class)
├── ordering-editor.blade.php (+ Livewire class)           // مع DnD
├── matching-editor.blade.php (+ Livewire class)
├── drag-drop-editor.blade.php (+ Livewire class)           // Canvas + DnD
├── hotspot-editor.blade.php (+ Livewire class)            // Canvas + النقر
├── short-answer-editor.blade.php (+ Livewire class)
├── essay-editor.blade.php (+ Livewire class)              // مع Rubric Builder
├── math-latex-editor.blade.php (+ Livewire class)          // KaTeX preview
├── code-question-editor.blade.php (+ Livewire class)       // Monaco Editor
├── media-question-editor.blade.php (+ Livewire class)
├── case-study-editor.blade.php (+ Livewire class)          // Sub-questions list

// components/grading/
├── rubric-grader.blade.php (+ Livewire class)             // واجهة تصحيح Essay
├── auto-grade-indicator.blade.php (+ Livewire class)       // أيقونة "تم التصحيح آلياً"
├── partial-credit-badge.blade.php (+ Livewire class)

// components/certificate/
├── certificate-preview.blade.php (+ Livewire class)       // معاينة قبل الإصدار
├── certificate-status-badge.blade.php (+ Livewire class)   // pending/active/revoked
├── qr-verify-button.blade.php (+ Livewire class)           // زر "ابحث عن شهادة"

// components/exam-taking/
├── exam-timer.blade.php (+ Livewire class)                // مؤقت تنازلي
├── question-navigator.blade.php (+ Livewire class)        // أرقام الأسئلة (المُجاب/الفارغ)
├── submit-confirm-dialog.blade.php (+ Livewire class)
├── security-warning-banner.blade.php (+ Livewire class)    // إنذارات tab blur وغيرها
```
...
```typescript
// lib/ai/cLaudeQuestionGenerator.php
...
| **Laravel Jobs / Scheduled Tasks** | توليد PDF للشهادات (في بيئة Lambda) |
...
### 7.4 Browsershot (spatie/browsershot) / Chromium

> **خيار الاستضافة:** بدلاً من تشغيل Browsershot (spatie/browsershot) كاملاً على VPS KSA (Hostinger / Contabo) (محدود)، نُهيِّء **Edge Function منفصل على Laravel + PostgreSQL** (`functions/render-certificate-pdf`) يستخدم `@sparticuz/chromium`. تكلفته أقل وأداؤه أسرع.
...
### 7.6 Laravel Scheduler Jobs

```typescript
// app/api/cron/cleanup-expired-attempts/rOute.php
...
{
    $schedule->job(new CleanupExpiredAttemptsJob)->hourly();
    $schedule->job(new ItemAnalysisJob)->dailyAt('02:00');
    $schedule->job(new CertStatusSyncJob)->everyThirtyMinutes();
}
```
...
|---------|---------------------------|
| **تشفير at-rest** | Postgres SSL + Storage encrypted by Laravel + PostgreSQL |
...
| **Injection** | Parameterized queries (Laravel + PostgreSQL Client) + Laravel Validation للـvalidation |
...
| **Vulnerable Components** | Dependabot + `composer audit` في CI |
...
```
        ▲
       ╱ ╲
      ╱E2E╲          25 سيناريو Laravel Dusk
     ╱─────╲
    ╱ Integ ╲       50 اختبار تكاملي (API + DB)
   ╱─────────╲
  ╱   Unit    ╲    150+ unit test (Pest)
 ╱─────────────╲
```
...
```typescript
// __tests__/grading/grade-mcq-multi.Test.php
...
```typescript
// __tests__/integration/exam-flow.Test.php
...
| 1.4 | إعداد Form Requests + Validation Rules للـ14 نوع | 1.5 | Backend |
...
| 4.7 | Browsershot (spatie/browsershot) setup + Laravel Job / Scheduled Task | 1.5 |
...
|---|----------|----------|--------|----------------------|---------|
| **R1** | **Browsershot (spatie/browsershot) لا يعمل على VPS KSA (Hostinger / Contabo) Serverless** | متوسط | عالي | 🔴 عالٍ | استخدام `@sparticuz/chromium` + Edge Function على Laravel + PostgreSQL. **بديل:** خادم VPS مخصص (5$/شهر) |
...
| **R8** | **ذاكرة PostgreSQL على VPS (500MB) لا تكفي** | عالٍ | منخفض | 🟡 متوسط | الانتقال لـPro ($25/شهر) عند الحاجة + Archive الأسئلة الـold |
...
|------|---------|----------------------|
| VPS KSA (Hostinger / Contabo) | $25 | $75 (~281 ريال) |
| VPS KSA (Hostinger / Contabo) Pro | $20 | $60 (~225 ريال) |
...
| 8. Test Results | Laravel Dusk HTML report + Coverage report | QA |
...
sequenceDiagram
    ...
    participant PDF as PDF Engine (Browsershot (spatie/browsershot))
    participant STO as Spatie Media Library
...
- [ ] Cron Jobs مجدولة في VPS KSA (Hostinger / Contabo).
- [ ] `.env` كامل في VPS KSA (Hostinger / Contabo) + Laravel + PostgreSQL Secrets.
...
### Operations
- [ ] Monitoring dashboards (Laravel Pulse / Telescope + Laravel Telescope / Logs).
