...
الهدف الرئيسي من هذه المرحلة هو **إنتاج نواة معمارية مستقرة وقابلة للتوسع** تخدم 1,000 طالب موزّعين على 4 فروع (BRANCH_A, BRANCH_B, BRANCH_C, BRANCH_D)، مع جاهزية معمارية للتوسع إلى 10,000 طالب و8 فروع دون إعادة كتابة جوهرية. سنُنشئ مشروع Laravel 11 + PHP 8.3 (Strict)، نُهيّئ مشروع Laravel + PostgreSQL مع Schema أولي يضمّ الجداول الأساسية (`branches`, `users`, `roles`, `permissions`, `role_permissions`, `audit_log`, `notifications`)، نُفعّل **Row-Level Security (RLS)** بسياسات صارمة على كل جدول، نُعدّ 4 أدوار قاعدية (`super_admin`, `admin`, `teacher`, `student`)، نُقيم خط CI/CD آلي على GitHub Actions يُغطي PR Checks وStaging وProduction، ونُفعّل Sentry + Laravel Pulse / Telescope للمراقبة.

السمة الفارقة لهذه المرحلة هي **Multi-Branch by Design**: كل جدول في قاعدة البيانات يحتوي عمود `branch_id`، وكل سياسة RLS تتحقق من انتماء المستخدم للفرع، وكل استعلام في الـ Blade Components يمر عبر طبقة موحّدة تضمن العزل. هذا يعني أن مدير الفرع الأول لن يتمكن **تقنياً** من قراءة بيانات الفرع الثاني — ليس لأن الواجهة تخفيها، بل لأن قاعدة البيانات ترفض إعادتها أصلاً. هذا قرار حاسم نتبناه من اليوم الأول لأن إصلاحه لاحقاً يكلف 5-10 أضعاف.

في نهاية الأسبوعين، سنُسلّم: **مستودع Git** كامل بمعمارية Feature-Based، **بيئة Laravel + PostgreSQL Staging** تعمل، **خط CI/CD** يُشغّل الاختبارات تلقائياً، **صفحة تسجيل دخول** عربية RTL مكتملة تقبل 4 أدوار، **لوحة تحكم فارغة** لكل دور تثبت أن المصادقة تعمل، **3 مستخدمين تجريبيين** يثبتون عزل الفروع، و**وثيقة قبول (UAT)** موقّعة من العميل. هذا التسليم وحده ليس "منتجاً" — لكنه **الأساس الذي يجعل كل المراحل القادمة ممكنة ومتوقعة زمنياً وتقنياً**.
...
| O7 | رصد فوري للأخطاء والأداء | Sentry يلتقط 100% من الأخطاء + Laravel Pulse / Telescope |
...
|---|----------|--------------|--------------|
| D1 | مستودع Git (GitHub) جاهز | `github.com/iims/iims` | حماية على `main` + `develop`، Branch Protection Rules |
| D2 | مشروع Laravel 11 + PHP 8.3 (Strict) | `apps/web` (single repo) | `php artisan optimize` ينجح بلا تحذيرات |
| D3 | بنية مجلدات Feature-based | `app/Filament/Resources/`, `app/Livewire/`, `app/Services/` | كل ميزة في مجلدها (auth, branches, users, audit) |
| D4 | مشروع Laravel + PostgreSQL (Cloud) | `iims-staging.iims-staging.example.sa` | Schema + RLS + Seed data مطبّقة |
| D5 | Migrations مطبقة | `database/migrations/0001_*.sql` إلى `0010_*.sql` | جميعها تعمل عبر `php artisan migrate:fresh --seed` |
...
| D8 | Sentry + Laravel Pulse / Telescope | Dashboard مُفعّل | يلتقط خطأ تجريبي مزروع |
...
| D11 | Middleware للحماية | `app/Http/Middleware/` | يُعيد توجيه غير المصرّح لهم لـ `/403` |
...
| D16 | E2E Tests أساسية | `tests/Browser/AuthTest.php` | 3 سيناريوهات: login/logout/wrong-role |
...
- ❌ PostgreSQL ذاتي الاستضافة على VPS سعودي (يُؤجَّل لمرحلة 9 أو بعد قبول العميل).
- ❌ تسجيل دخول الطلاب عبر OTP/SMS موسّع (المرحلة 9).
...
| حساب حساب VPS KSA | المطوّر | Linked to GitHub Org |
| حساب Laravel + PostgreSQL | المطوّر | Free Tier + Staging Project مُنشأ |
| حساب Cloudflare | المطوّر | Domain مُسجّل (مثلاً `iims.sa` أو فرعي للاختبار) |
...
|--------|----------------|------|
| Node.js | 20.10 LTS | تشغيل Laravel |
| composer | 9.x | مدير الحزم (نختار composer لكفاءة الـ disk) |
| Git | 2.40+ | التحكم بالإصدارات |
| Docker Desktop | 4.30+ | تشغيل Laravel Sail Local |
| Laravel Artisan | 1.190+ | Migrations + Local Dev |
| VS Code + Extensions | latest | محرر مع Laravel Pint, Laravel Pint, Tailwind, Deno |
...
| **بريد إلكتروني رسمي للنظام** (مثلاً `noreply@iims.sa`) | 🟠 عالية | اليوم 4 |
...
- **Single Repo** بدلاً من Monorepo معقد (composer workspaces ممكن لاحقاً عند ظهور `packages/shared`).
- **Laravel Routes + Filament Panels** في Laravel 11 (لا Web Routes (routes/web.php)).
- **PostgreSQL على VPS KSA** للبداية، Self-Hosted KSA مؤجَّل.
- **PHPStan Level 8 (Larastan)** إلزامي (لا `any`، لا `enum`).
- **Conventional Commits** + Lefthook / Git Hooks.
...
إنشاء البنية التحتية للكود: المستودع، Laravel، PHP 8.3، Tooling، بنية المجلدات. هذا أول ما يبدأ به المطوّر في اليوم الأول.

#### 4.1.2 User Stories

- **US-1.1.1** كمطوّر، أريد `composer install && php artisan serve` يعمل على جهاز جديد خلال أقل من 10 دقائق، لأبدأ التطوير فوراً.
- **US-1.1.2** كمطوّر، أريد Laravel Pint و Laravel Pint يعملان تلقائياً عند الحفظ في VS Code، لتجنّب أخطاء التنسيق.
- **US-1.1.3** كمطوّر، أريد Git Hooks (Lefthook / Git Hooks) تمنعني من commit بكود لا يجتاز lint+typecheck، لأحمي الفرع الرئيسي.
...
| Package Manager | composer 9.x (محدد في `composer.json` → `packageManager`) |
| Laravel | 14.2+ (Laravel Routes + Filament Panels) |
| PHP 8.3 | 5.4+ Strict mode (راجع `phpstan.neon` في الجزء 04) |
| Laravel Pint | 8.57+ مع `laravel/pint` + `pint-rule-tailwindcss` |
| Laravel Pint | 3.3+ مع `pint-tailwindcss` |
| Lefthook / Git Hooks | 9.1+ + lint-staged |
...
لا يوجد بعد. سيُعَدّ الـ infrastructure للـ Livewire Actions / Laravel Controllers فقط (folder `app/Http/Controllers/` + `app/Livewire/`).
...
- **Smoke Test**: `php artisan serve` يفتح `localhost:3000` ويعرض الصفحة الافتراضية بـ status 200.
- **Build Test**: `php artisan optimize` ينجح بلا أخطاء أو تحذيرات.
- **Lint Test**: `composer pint` ينجح.
- **Type Check**: `composer phpstan` ينجح.
...
│   ├── Exceptions/
│   │   └── Handler.php
│   ├── Filament/                           # Filament Resources + Panels
│   │   ├── Admin/
│   │   │   ├── Resources/                  # User, Branch, Role, AuditLog
│   │   │   ├── Pages/
│   │   │   └── Widgets/
│   │   ├── Branch/
│   │   ├── Teacher/
...
│   │   ├── AuditLog.php
│   │   └── Notification.php
│   ├── Policies/                           # Laravel Policies (RLS replacement)
│   │   ├── BranchPolicy.php
│   │   ├── UserPolicy.php
│   │   └── AuditLogPolicy.php
│   ├── Providers/
...
│   │   └── Notifications/NotifyService.php
│   ├── Support/
│   │   ├── Env.php                         # Validated env helpers
│   │   └── helpers.php
│   └── Jobs/                               # Laravel Horizon Jobs (Redis)
├── bootstrap/
│   ├── app.php
│   └── cache/
├── config/                                 # ملفات الإعداد (auth, database, filament, queue ...)
│   ├── app.php
│   ├── auth.php
│   ├── database.php
│   ├── filament.php
...
├── database/
│   ├── migrations/
│   │   ├── 2026_05_13_000001_create_branches_table.php
│   │   ├── 2026_05_13_000002_create_users_table.php
│   │   ├── 2026_05_13_000003_create_roles_table.php
│   │   ├── 2026_05_13_000004_create_permissions_table.php
│   │   ├── 2026_05_13_000005_create_role_permissions_table.php
│   │   ├── 2026_05_13_000006_create_audit_log_table.php
│   │   ├── 2026_05_13_000007_create_notifications_table.php
│   │   └── 2026_05_13_000008_create_login_attempts_table.php
│   ├── seeders/
│   │   ├── DatabaseSeeder.php
│   │   ├── BranchSeeder.php
│   │   ├── RoleSeeder.php
...
│   ├── js/
│   │   └── app.js                          # Alpine.js + Livewire + Echo (Reverb)
│   ├── lang/ar/                            # ترجمات عربية
│   └── views/
│       ├── components/                     # Blade Components
│       ├── filament/                       # Filament custom Blade views
...
│       ├── emails/                         # قوالب البريد
│       ├── layouts/
│       │   └── app.blade.php               # HTML lang="ar" dir="rtl"
│       ├── errors/
│       │   ├── 403.blade.php
│       │   ├── 404.blade.php
│       │   └── 500.blade.php
...
├── .env.example
├── .gitignore
├── .nvmrc                                  # Node 20 LTS (للأصول فقط)
├── artisan
├── composer.json
├── composer.lock
├── docker-compose.yml                      # Laravel Sail
├── package.json                            # Vite + Tailwind (assets only)
├── phpstan.neon                            # Larastan rules
...
#### 4.1.9 ملف `composer.json` الأساسي

```json
{
  "name": "iims-org/iims",
  "type": "project",
  "description": "النظام IIMS — Integrated Institute Management System",
  "license": "proprietary",
  "require": {
    "php": "^8.3",
  ...
  "scripts": {
    ...
    "test:e2e:ui":  ["@php artisan dusk --browse"],
    "db:fresh":     ["@php artisan migrate:fresh --seed"],
    "db:migrate":   ["@php artisan migrate"],
    "db:seed":      ["@php artisan db:seed"],
    "validate-all": ["@lint", "@phpstan", "@test"]
  }
}
```

وملف `package.json` الموازي (للأصول فقط عبر Vite):
...
SESSION_LIFETIME=120                       # 2 hours sliding
...
# ==================================================
# Filesystem / Spatie Media Library
# ==================================================
...
# ==================================================
# Mail (Resend via resend/resend-laravel)
# ==================================================
MAIL_MAILER=resend
RESEND_API_KEY=
MAIL_FROM_ADDRESS="noreply@iims.example.sa"
MAIL_FROM_NAME="${APP_NAME}"

# ==================================================
# SMS / WhatsApp (Unifonic via Laravel HTTP)
# ==================================================
UNIFONIC_APP_SID=
UNIFONIC_SENDER_ID=
UNIFONIC_BASE_URL=https://api.unifonic.com

# ==================================================
# AI (Claude API via Laravel HTTP Client)
# ==================================================
ANTHROPIC_API_KEY=
...
# Security
# ==================================================
JWT_SECRET=                                # for non-Laravel-auth tokens (TVTC integration)
ENCRYPTION_KEY=                            # 32-byte hex key for pgcrypto column-level

# ==================================================
# Feature Flags
# ==================================================
FEATURE_2FA_ENFORCED=true
FEATURE_AUDIT_UI=true
FEATURE_MAINTENANCE_MODE=false
```

#### 4.1.11 `app/Support/Env.php` — التحقق من صحة متغيرات البيئة
...
{
    /**
     * تحقّق من متغيرات البيئة الإلزامية عند bootstrap (في AppServiceProvider::boot()).
     * أي قيمة ناقصة أو غير صحيحة → exception فوراً، فلا تُشغَّل التطبيق على إعداد ناقص.
     */
    public static function validate(): void
    {
        $validator = Validator::make(
            ...
            rules: [
                ...
                'APP_TIMEZONE'  => ['required', 'in:Asia/Riyadh'],
                'APP_LOCALE'    => ['required', 'in:ar'],
        ...
        if ($validator->fails()) {
            throw new \RuntimeException(
                'متغيرات البيئة غير صالحة: ' . $validator->errors()->first()
            );
        }
    }
}
...
### 4.2 إعداد Laravel + PostgreSQL والـ Schema الأولي

#### 4.2.1 الوصف

إنشاء مشروع PostgreSQL على VPS KSA (Free Tier للبداية)، إعداد Laravel Artisan محلياً، كتابة Migrations للجداول السبعة الأساسية، إعداد Seed data للفروع الأربعة. هذه نواة قاعدة البيانات التي ستُضاف لها 40+ جدولاً في المراحل القادمة.

#### 4.2.2 User Stories

- **US-1.2.1** كمطوّر، أريد `php artisan sail:up` يُشغّل Postgres محلياً مع كل الـ Schemas الأخيرة، لأطوّر دون قلق من تأثير زملائي.
...
- **Laravel Artisan** على جهاز المطوّر.
- **Project** على PostgreSQL على VPS KSA (region: `eu-central-1` أو الأقرب).
...
#### 4.2.5 الـ APIs / Livewire Actions / Laravel Controllers

- `app/Models/` + `app/Providers/DatabaseServiceProvider.php` — عميل Laravel + PostgreSQL للـ Blade Components.
- `config/database.php` + `app/Providers/DatabaseServiceProvider.php` — عميل Laravel + PostgreSQL للـ Livewire Components.
- `app/Http/Middleware/AuthenticateSession.php` — عميل خاص بـ Middleware (Refresh Session).
- `app/Models/` (Eloquent Models) — Types مُولّدة تلقائياً من `php artisan ide-helper:models`.
...
- **Migration Test**: `php artisan migrate:fresh --seed` يُطبّق كل الـ Migrations بنجاح.
...
- **Types Test**: `php artisan ide-helper:models` يُنتج ملف PHP 8.3 بلا أخطاء.
...
تفعيل Laravel Auth + Spatie Permission، إعداد تسجيل الدخول بالبريد + كلمة المرور، تفعيل 2FA (TOTP) للأدوار الحساسة، إنشاء 4 أدوار قاعدية، middleware للحماية، Session Management، Lockout policy.
...
| البند | المواصفة |
|------|----------|
| Provider | Laravel Auth (Sanctum + Spatie Permission) |
...
| Hash | Argon2id (افتراضي Laravel + PostgreSQL) |
| Session JWT | 1 ساعة |
...
- جدول `auth_identities` (موجود في الجزء 03) — مُؤجَّل للمرحلة 9 (OTP/SMS موسّع).

#### 4.3.5 الـ APIs / Livewire Actions / Laravel Controllers

| الـ Action / Endpoint | الغرض | الـ Path |
|----------------------|-------|---------|
| `signInWithPassword` | تسجيل الدخول | Livewire Action / Laravel Controller: `app/Livewire/auth/server/sIgnIn.php` |
| `signOut` | تسجيل الخروج | Livewire Action / Laravel Controller: `app/Livewire/auth/server/sIgnOut.php` |
| `enroll2FA` | تفعيل 2FA لأول مرة | Livewire Action / Laravel Controller: `app/Livewire/auth/server/eNroll2fa.php` |
| `verify2FA` | إدخال كود 2FA | Livewire Action / Laravel Controller: `app/Livewire/auth/server/vErify2fa.php` |
| `requestPasswordReset` | طلب إعادة كلمة المرور | Livewire Action / Laravel Controller: `app/Livewire/auth/server/rEquestReset.php` |
| `resetPassword` | تعيين كلمة المرور الجديدة | Livewire Action / Laravel Controller: `app/Livewire/auth/server/rEsetPassword.php` |
| `getCurrentUser` | جلب المستخدم الحالي | Eloquent Query (Service / Repository): `app/Livewire/auth/server/gEtCurrentUser.php` |
| `GET /api/health` (Laravel API Route) | فحص الصحة | Laravel Controller |

#### 4.3.6 مكوّنات الـ UI

- **`<LoginForm>`** — نموذج تسجيل دخول بـ Filament Forms / Livewire Forms + Laravel Validation.
...
- **Unit Tests** على Form Requests + Validation Rules (`signInSchema`, `resetPasswordSchema`).
- **Integration Tests** على Livewire Actions / Laravel Controllers (`signInWithPassword` تُرجع خطأ مع كلمة خاطئة).
...
- **RLS Test Suite** في `tests/integration/rls/`:
  - `branches.Test.php`: super_admin يرى 4 فروع، admin يرى 4 فروع، teacher يرى فرعه فقط (إن وُجد)، student يرى فرعه فقط.
  - `users.Test.php`: student لا يرى student آخر، teacher لا يرى teacher آخر إلا في نفس الفرع.
  - `audit_log.Test.php`: محاولة DELETE تفشل لكل الأدوار.
  - `cross_branch.Test.php`: مستخدم في BRANCH_A لا يستطيع SELECT من جدول students حيث branch_id = BRANCH_B.
...
#### 4.5.5 الـ APIs / Livewire Actions / Laravel Controllers

- `getActiveBranches()` — Eloquent Query (Service / Repository) تُرجع الفروع النشطة (للعرض في dropdowns لاحقاً).
- `getMyBranch()` — Eloquent Query (Service / Repository) تُرجع فرع المستخدم الحالي.
...
#### 4.6.4 الـ APIs / Livewire Actions / Laravel Controllers
...
- **`<AuditLogTable>`** — جدول رئيسي مع `Filament Tables`.
...
- **Tailwind Plugin**: `tailwindcss-rtl (نفسه)` مُفعَّل.
- **Cairo Font**: محلي في `public/fonts/` (4 وزنات: 400, 500, 600, 700).
- **Logical Properties**: قاعدة Laravel Pint تمنع استخدام `mr-`, `ml-`, `pr-`, `pl-` (custom rule أو pint-rule-tailwindcss config).
...
// Cairo font loaded via @vite directive في Blade in app.blade.php or fonts.bunny.net
...
export default function RootLayout({ children }: { children: View }) {
  return (
...
#### 4.7.5 إعداد `tailwind.config.js`
...
const config: Config = {
  ...
  plugins: [require('tailwindcss-rtl (نفسه)')],
};
...
- **Lint Test**: قاعدة Laravel Pint تمنع `mr-`, `ml-`, `pr-`, `pl-`.
...
جدول `notifications` + API endpoint + Component للعرض + Laravel Reverb (WebSockets) subscription. هذا "هيكل" فقط — المنطق التجاري (متى نُنشئ إشعارات) سيأتي في المراحل اللاحقة.
...
- Laravel Reverb (WebSockets) مُفعّل على `notifications`.
- Toast (Filament Notifications) عند وصول إشعار جديد.
...
#### 4.8.5 الـ APIs / Livewire Actions / Laravel Controllers
...
3 GitHub Actions Workflows: PR Checks (lint + typecheck + test + build)، Auto-deploy Staging عند merge إلى `develop`، Auto-deploy Production عند merge إلى `main` (مع موافقة يدوية). Laravel + PostgreSQL Migrations تُطبَّق آلياً.
...
- **Forge CLI** للنشر.
- **Laravel Artisan** للـ Migrations.
...
flowchart LR
    ...
    STG --> VPS_STG[نشر VPS Staging]
    VPS_STG --> UAT[UAT Testing]
    ...
    PROD --> VPS_PROD[نشر VPS Production]
```
  ...
  deploy:
    ...
    steps:
      ...
      - uses: composer/action-setup@v4
        with: { version: 9 }
      - uses: actions/setup-node@v4
        with: { node-version: 20, cache: 'composer' }
      - run: composer install --no-dev --optimize-autoloader

      # Apply Laravel + PostgreSQL migrations عبر SSH على VPS
...
> **بديل أبسط:** استخدام **Laravel Forge** لإدارة الـ Deployments (يربط GitHub بـ VPS مباشرة، Zero-Downtime، SSL تلقائي، Daemons، Scheduled Jobs).
...
jobs:
  deploy:
    ...
    steps:
      # ... similar to staging but with production secrets + manual approval gate
      - name: SSH Deploy to Production VPS (Zero-Downtime)
        uses: appleboy/ssh-action@v1.0.3
        with:
          host: ${{ secrets.PRODUCTION_VPS_HOST }}
          username: ${{ secrets.PRODUCTION_VPS_USER }}
          key: ${{ secrets.PRODUCTION_VPS_SSH_KEY }}
          script: cd /var/www/iims && php artisan deploy:zero-downtime
```
...
| Secret | البيئة | الغرض |
|--------|--------|------|
| `STAGING_VPS_HOST` | staging | عنوان VPS Staging |
| `STAGING_VPS_USER` | staging | SSH user |
| `STAGING_VPS_SSH_KEY` | staging | Private key للنشر |
| `STAGING_DB_HOST` | staging | PostgreSQL host |
| `STAGING_DB_DATABASE` | staging | اسم DB |
| `STAGING_DB_USERNAME` | staging | DB user |
| `STAGING_DB_PASSWORD` | staging | DB password |
| `STAGING_APP_KEY` | staging | Laravel APP_KEY |
| `PRODUCTION_VPS_HOST` | production | عنوان VPS Production |
| `PRODUCTION_VPS_USER` | production | SSH user |
| `PRODUCTION_VPS_SSH_KEY` | production | Private key |
| `PRODUCTION_DB_HOST` | production | PostgreSQL host |
| `PRODUCTION_DB_DATABASE` | production | اسم DB |
| `PRODUCTION_DB_USERNAME` | production | DB user |
| `PRODUCTION_DB_PASSWORD` | production | DB password |
| `PRODUCTION_APP_KEY` | production | Laravel APP_KEY |
| `FORGE_API_TOKEN` | جميع | (اختياري) Laravel Forge API |
| `SENTRY_AUTH_TOKEN` | جميع | Source Maps Upload |

#### 4.9.7 الاختبارات المطلوبة

- **PR Test**: فتح PR بكود يحتوي خطأ Laravel Pint → الـ Action تفشل.
...
Sentry لرصد الأخطاء، Laravel Pulse / Telescope للأداء، Filament Admin Panel لـ DB metrics، استراتيجية Logging (Pino + Laravel Telescope / Logs).
...
- **Sentry SDK** `sentry/sentry-laravel` مُفعّل في الـ Server + Client.
- **Laravel Pulse + Telescope** لرصد الـ slow queries، الـ scheduled jobs، الـ exceptions، والـ HTTP requests (مدمج، بدون اعتمادية خارجية).
- **Logger** مخصص (Pino) في `config/logging.php` + `Log::channel(...)`.
- **Health Endpoint** `/api/health` (Laravel Route) يُرجع status DB + Auth.

#### 4.10.4 إعداد Sentry

```typescript
// config/sentry.php
// Sentry configured via config/sentry.php (sentry/sentry-laravel)

// Configured via config/sentry.php and SENTRY_LARAVEL_DSN env
Sentry::init({
...
```typescript
// config/logging.php
import Monolog (Laravel Logs) from 'Monolog (Laravel Logs)';

export const logger = Monolog (Laravel Logs)({
  level: process.env.NODE_ENV === 'production' ? 'info' : 'debug',
  transport:
    process.env.NODE_ENV !== 'production'
      ? { target: 'Laravel Logs (Monolog pretty)', options: { colorize: true } }
      : undefined,
  base: {
    app: 'iims',
...
- **Health Test**: GET `/api/health` (Laravel Route) يُرجع 200 + JSON `{ db: 'ok', auth: 'ok' }`.
...
- **HSTS Headers** + TLS 1.3 (تلقائي على VPS KSA (Hostinger / Contabo)).
- **AES-256** للـ Storage (افتراضي Laravel + PostgreSQL).
...
#### 4.11.5 الـ APIs / Livewire Actions / Laravel Controllers
...
```sql
-- File: database/migrations/20260513000001_branches.sql
...
```sql
-- File: database/migrations/20260513000002_users.sql
...
```sql
-- File: database/migrations/20260513000003_roles.sql
...
```sql
-- File: database/migrations/20260513000004_permissions.sql
...
```sql
-- File: database/migrations/20260513000005_role_permissions.sql
...
```sql
-- File: database/migrations/20260513000006_audit_log.sql
...
```sql
-- File: database/migrations/20260513000007_notifications.sql
...
COMMENT ON TABLE notifications IS 'الإشعارات الداخلية في النظام (مدفوعة عبر Laravel Reverb broadcasts)';
```

#### 5.2.8 Migration 0008: Login Attempts

```sql
-- File: database/migrations/20260513000008_login_attempts.sql
...
```sql
-- File: database/migrations/20260513000009_rls_policies.sql
...
-- User can update their own non-critical fields (handled via Livewire Action / Laravel Controller)
...
-- Admin sees everyone (read only here; mutations via Livewire Actions / Laravel Controllers only)
...
```sql
-- File: database/migrations/20260513000010_triggers.sql
...
```sql
-- File: database/migrations/20260513000011_seed.sql
...
flowchart TD
    ...
    B --> C[إدخال البريد]
    C --> D[Livewire Action / Laravel Controller: إرسال رابط]
...
- **Loading States**: كل Livewire Action / Laravel Controller له `<Spinner>` أو Skeleton.
- **Error States**: كل خطأ يظهر بـ Toast (Filament Notifications) أو inline.
- **Empty States**: كل قائمة فارغة لها رسالة + CTA (مثلاً: "لا يوجد مستخدمون. أنشئ أول مستخدم").
- **Confirmation**: كل عملية حذف أو حساسة تتطلب AlertDialog (Filament 3 component).
- **Accessibility**:
  - كل صورة لها `alt`.
...
في هذه المرحلة (التأسيس)، **لا توجد تكاملات خارجية فعلية**. كل التكاملات (Accounting API, WhatsApp, SMS, OTP/SMS) مؤجَّلة للمراحل اللاحقة.
...
```typescript
// app/Mail/sEndPasswordReset.php
use Resend\Laravel\Facades\Resend;
import { env } from '@/shared/lib/env';

const resend = Resend::client(env.RESEND_API_KEY);
...
```typescript
// tests/integration/rls/branch-isolation.Test.php
...
graph TB
    ...
    B --> C[VPS KSA - HSTS - CSP]
    C --> D[Laravel Middleware<br/>RBAC + Session Check]
    D --> E[Livewire Action / Laravel Controller<br/>Laravel Validation Validation]
    E --> F[Laravel + PostgreSQL Client<br/>JWT in Header]
...
2. **VPS KSA**: Security Headers (HSTS, CSP, X-Frame-Options).
3. **Middleware**: التحقق من وجود Session + Role قبل عرض الصفحة.
4. **Livewire Action / Laravel Controller**: Laravel Validation (Form Requests) + Business Logic Authorization.
5. **Laravel + PostgreSQL Client**: JWT في Authorization header (تلقائي).
...
| **SQL Injection** | منخفضة | حرج | Laravel + PostgreSQL Client (parameterized) + Laravel Validation |
| **XSS** | متوسطة | عالي | Livewire/Blade يهرب القيم تلقائياً + CSP |
...
| Integration Tests | كل Livewire Action / Laravel Controller مغطى | 100% للأهم |
...
```typescript
// tests/unit/lib/validators.Test.php
import { describe, it, expect } from 'pest';
...
```typescript
// tests/integration/auth/sign-in.Test.php
describe('Sign In Livewire Action / Laravel Controller', () => {
  beforeEach(async () => {
    await resetDb();
    await seedUser({
  ...
  it('records failed attempt in login_attempts', async () => {
    ...
    const { data } = await dbServiceRole
      .from('login_attempts')
      .select('*')
...
import { test, expect } // Pest PHP via use Tests\DuskTestCase;
...
```typescript
// tests/integration/rls/_Helpers.php
export async function asUser(role: BaseRole, branchCode?: string) {
  const { data: { session } } = await Auth::attempt({
...
| PR Check | Integration (Laravel Sail Local) | ~5 دقائق |
...
| 10:00-11:00 | تهيئة Laravel 11 + PHP 8.3 (Strict) | `composer create next-app` | 1س |
| 11:00-12:30 | إعداد Laravel Pint + PHPStan/Larastan + Lefthook / Git Hooks + Commitlint | `hooks/` + configs | 1.5س |
| 13:30-15:00 | بنية المجلدات Feature-based | `app/Livewire/` | 1.5س |
| 15:00-16:30 | `.env.example` + Laravel Validation (Form Requests) | `Env.php` | 1.5س |
| 16:30-17:00 | First Commit + Push | Initial commit | 30د |

**Definition of Done:**
- `composer install && php artisan serve` يعمل.
- `composer pint && composer phpstan && php artisan optimize` ينجح.
- Git Hooks تمنع commit بكود فاشل.

#### اليوم 2: Laravel + PostgreSQL Setup + Schema

| الوقت | المهمة | المخرج |
|------|--------|--------|
| 09:00-10:00 | إنشاء مشروع Laravel + PostgreSQL + Link CLI | Project URL |
...
| 16:30-17:00 | `php artisan migrate:fresh --seed` يعمل | Local DB |

**Definition of Done:**
- 4 فروع موجودة في DB.
- 4 أدوار + Permissions seeded.
- `php artisan ide-helper:models` ينتج types.
...
| 13:30-14:30 | تطبيق Tailwind + tailwindcss-rtl (نفسه) plugin | `tailwind.config.js` |
| 14:30-15:30 | تنزيل خط Cairo محلياً + Bunny Fonts / Vite Font Pipeline | `resources/views/layouts/app.blade.php` |
| 15:30-17:00 | Filament 3 UI init + 5 مكونات أساسية (Button, Input, Label, Card, Toast) | `components/ui/` |

**Definition of Done:**
- RLS مُفعَّلة على كل الجداول.
- خط Cairo يعمل.
- 5 Filament 3 component components تعمل.

#### اليوم 4: Authentication + Middleware

| الوقت | المهمة | المخرج |
|------|--------|--------|
| 09:00-10:30 | Laravel + PostgreSQL Client setup (Server.php, Client.php, Middleware.php) | 3 files |
| 10:30-12:30 | صفحة `/login` كاملة (UI + Livewire Action / Laravel Controller) | `resources/views/livewire/auth/login.blade.php` + `app/Livewire/Auth/LoginForm.php` |
| 13:30-15:00 | `app/Http/Middleware/` (RBAC routing) | `Middleware.php` |
| 15:00-16:00 | Lockout Policy + login_attempts | Livewire Action / Laravel Controller |
...
| الوقت | المهمة | المخرج |
|------|--------|--------|
| 09:00-10:30 | `getAuditLogs` Eloquent Query (Service / Repository) | `server/queries/aUditLog.php` |
...
| 10:30-12:30 | `<NotificationDropdown>` + Livewire Actions / Laravel Controllers | Component |
| 13:30-15:00 | Realtime subscription | `<RealtimeNotificationListener>` |
| 15:00-16:30 | Toast (Filament Notifications) عند إشعار جديد | Integration |
...
| 15:00-16:00 | Laravel + PostgreSQL Migrations في CI | Integration |
| 16:00-17:00 | GitHub Environments + Secrets | Settings |

**Definition of Done:**
- PR بـ Laravel Pint error → الـ CI تفشل.
...
| الوقت | المهمة | المخرج |
|------|--------|--------|
| 09:00-10:30 | Sentry setup (client + server) | `sentry.*.Config.php` |
| 10:30-11:30 | Laravel Pulse / Telescope + Speed Insights | `resources/views/layouts/app.blade.php` |
| 11:30-12:30 | Pino Logger في الـ Livewire Actions / Laravel Controllers | `lib/lOgger.php` |
| 13:30-14:30 | `/api/health` (Laravel Route) endpoint | Laravel Controller |
...
- `/api/health` (Laravel Route) يُرجع 200.
...
| الوقت | المهمة | المخرج |
|------|--------|--------|
| 09:00-11:00 | E2E Tests أساسية (auth flow, RTL, audit) | `e2e/*.Spec.php` |
...
| # | المعيار | الاختبار |
|---|---------|---------|
| AC-2.1 | `composer install && php artisan serve` يعمل على جهاز جديد | اختبار على VM/Docker |
| AC-2.2 | `php artisan optimize` ينجح بلا أخطاء | Run CI |
| AC-2.3 | `composer phpstan` ينجح | Run CI |
| AC-2.4 | `composer pint` ينجح بلا تحذيرات | Run CI |
| AC-2.5 | `php artisan test` ينجح بـ ≥ 80% coverage | Coverage Report |
| AC-2.6 | E2E tests الـ 5 الأساسية تمر | `php artisan dusk` |
...
| AC-2.11 | `/api/health` (Laravel Route) يُرجع 200 + JSON | curl |
...
| AC-3.5 | كل Input له validation Laravel Validation | فحص الكود |
| AC-3.6 | لا `console.log` في الكود الإنتاجي | Laravel Pint rule |
| AC-3.7 | لا `any` في PHP 8.3 | Laravel Pint rule |
| AC-3.8 | كل ميزة في `app/Livewire/<name>/` | فحص بصري |
| AC-3.9 | كل migration مرقّمة ومُؤرَّخة | `database/migrations/` |
...
| R2 | **PostgreSQL على VPS محدود (500MB DB, 2GB Bandwidth)** | متوسطة | منخفض | الترقية لـ Pro ($25/شهر) عند الحاجة في المرحلة 2 |
...
| R5 | **خط Cairo من Google Fonts بطيء** | متوسطة | منخفض | تنزيله محلياً في `public/fonts/` (Bunny Fonts / Vite Font Pipeline) |
...
| R13 | **PDPL لا يُتوافق بشكل كامل (PostgreSQL على VPS KSA في فرانكفورت)** | عالية | عالي | اعتبارها ملاحظة مؤجَّلة للمرحلة 9 (Self-Hosted KSA) + موافقة العميل المكتوبة |
| R14 | **التأخر في مراجعة العميل لكل sprint** | متوسطة | متوسط | جدولة Demo مع العميل في نهاية كل sprint مسبقاً |
| R15 | **تباين بين بيئة المطوّر المحلية والإنتاج** | متوسطة | متوسط | Laravel Sail Local Docker مطابق لـ Cloud + استخدام نفس الإصدارات |
...
| 1.2 Laravel + PostgreSQL Setup + Schema | 1.5 | حرج | 1.1 |
...
|--------|--------|---------|
| Laravel + PostgreSQL | Free | $0 |
| VPS KSA (Hostinger / Contabo) | Hobby | $0 (للتطوير) أو Pro $20 للإنتاج |
| Cloudflare | Free | $0 |
...
- [ ] Laravel + PostgreSQL Staging Project يعمل.
- [ ] 4 Branches Seeded.
- [ ] 4 Roles Seeded.
...
- [ ] Integration Tests للـ Livewire Actions / Laravel Controllers الأساسية.
...
- [ ] VPS KSA (Hostinger / Contabo) Production Deployment يعمل.
- [ ] VPS KSA (Hostinger / Contabo) Staging Deployment يعمل.
- [ ] Cloudflare DNS + WAF + SSL.
- [ ] Domain Custom مُعدَّ (مثلاً `iims.iims.sa`).
...
- [ ] JSDoc على Livewire Actions / Laravel Controllers.
...
| عدد الـ Livewire Actions / Laravel Controllers | 15+ |
| عدد الصفحات | 14+ |
...
4. **الـ Livewire Actions / Laravel Controllers vs Laravel Controllers** — Livewire Actions / Laravel Controllers للـ Mutations، Laravel Controllers فقط للـ APIs المُستهلكة من خارج التطبيق.
